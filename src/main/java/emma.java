...
public final class emma {
    ...
    private static final String USAGE = IAppConstants.APP_NAME_LC + " usage: " + IAppConstants.APP_NAME_LC
            + " <command> [command options]," + EOL + "  where <command> is one of:" + EOL + EOL
            + "   run     application runner {same as '" + IAppConstants.APP_NAME_LC + "run' tool};" + EOL
            + "   instr   offline instrumentation processor;" + EOL + "   ctl     remote control processor;" + EOL
            + "   merge   offline data file merge processor." + EOL + "   report  offline report generator;" + EOL + EOL
            + "  {use '<command> -h' to see usage help for a given command}" + EOL + EOL
