...
// ----------------------------------------------------------------------------
/**
 * Base class for all {@link IFileLock} implementations. Implements common retry
 * logic in {@link #acquire()} and delegates actual lock resource acquisition to
 * {@link #tryAcquire()}.
 * 
...
abstract class AbstractFileLock implements IFileLock {
    ...
    public synchronized final void acquire() throws IFileLock.FileLockException, InterruptedException {
        try {
            tryAcquire();
            return;
    ...
    /**
     * Called by {@link #acquire()} in an attempt to acquire this lock once. This
     * method does not block and throws an IFileLock.FileLockException on any
     * failure.
     * <P>
     * 
     * NOTE: failures with error codes {@link IFileLock#LOCK_ACQUIRED_ALREADY} and
     * {@link IFileLock#LOCK_ACQUISITION_SECURITY_FAILURE} abort any further
     * retries.
     * 
