 ...
 * TODO: javadoc
 * 
 * Based on code <a href=
 * "http://www.fawcette.com/javapro/2002_12/online/exception_vroubtsov_12_16_02/default_pf.asp">published</a>
...
abstract class ExceptionCommon implements IJREVersion {
     ...
     * resource bundle to the set that is used to look up error codes. This makes it
     * possible to extend the set of exception error codes across independently
     * maintained and built projects.
     * <P>
     *
     * <BLOCKQUOTE> Note that this introduces a possibility of error code name
     * clashes. This is resolved in the following way:
     * <UL>
     * <LI>when <CODE>getMessage(namespace, code)</CODE> is called, 'code' is
     * attempted to be looked up in the resource bundle previously keyed under
     * 'namespace';
     *
     * <LI>if no such bundle it found or if it does not contain a value for key
     * 'code', the same step is repeated for the superclass of 'namespace';
     *
     ...
     * <P>
     *
     * There is no visible state change if the indicated resource is not found or if
     * it has been added already under the same key.
     * <P>
     *
     * @param namespace                 the Class object acting as the namespace key
     *                                  for the resource bundle identified by
     *                                  'messageResourceBundleName'. <I>This should
     *                                  be the calling class.</I> [the method is a
     *                                  no-op if this is null]
     *
     * @param messageResourceBundleName name of a bundle (path relative to
     *                                  'namespace' package) to add to the set from
     *                                  which error code mappings are retrieved [the
     *                                  method is a no-op if this is null or an
     *                                  empty string]
     * 
     * @return ResourceBundle that corresponds to 'namespace' key or null if no such
     *         bundle could be loaded
     *
     * @throws Error if 'namespace' does not correspond to an exception class
     *               derived from {@link AbstractException} or
     *               {@link AbstractRuntimeException}.
     *
     * @see #lookup
     */
    public static ResourceBundle addExceptionResource(final Class namespace, final String messageResourceBundleName) {
        if ((namespace != null) && (messageResourceBundleName != null) && (messageResourceBundleName.length() > 0)) {
            ...
            if (!ABSTRACT_EXCEPTION.isAssignableFrom(namespace)
                ...
                throw new Error("addExceptionResource(): class [" + namespace + "] is not a subclass of "
                        + ABSTRACT_EXCEPTION.getName() + " or " + ABSTACT_RUNTIME_EXCEPTION.getName());
            }
     ...
     * {@link AbstractException} and {@link AbstractRuntimeException} and their
     * subclasses.
     *
     * @param namespace the Class object acting as the key to the namespace from
     *                  which to retrieve the description for 'code' [can be null,
     *                  in which case only the root namespace is used for lookup]
     *
     * @param code      the message string value that was passed into exception
     *                  constructor [can be null, in which case null is returned].
     *
     * @return looked-up error message or the error code if it could not be looked
     *         up [null is returned on null 'code' input only].
     *
    ...
    /**
     * Internal property lookup method. It implements the lookup scheme described in
     * {@link #addExceptionResource}.
     * 
    ...
    private static String lookup(Class namespace, final String propertyName) {
        ...
        if (namespace != null) {
            ResourceBundle rb;
            while (namespace != ABSTRACT_EXCEPTION && namespace != ABSTACT_RUNTIME_EXCEPTION && namespace != THROWABLE
                    && namespace != null) {
                synchronized (s_exceptionCodeMap) {
    ...
    // static cache of all loaded resource bundles, populated via
    ...
    static {
        ...
        try {
            ...
            // if the exception codes rb is absent, we are still in a supported
            // configuration
        }
