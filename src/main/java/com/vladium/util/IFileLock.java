...
// ----------------------------------------------------------------------------
/**
 * A simple abstraction for implementing mutual exclusivity across multiple OS
 * processes on the same physical host.
 * <P>
 * 
 * Lifecycle: create() -&gt; (acquire() -&gt; release() -&gt;)* discard
 * <P>
...
public interface IFileLock {
    ...
    /**
     * Releases this lock. This method may be called multiple times (it has no
     * effect on an unacquired/released lock).
     */
    void release();

    abstract class Factory {
        /**
         * Creates a new lock for the canonical filepath represented by 'file'.
         * <P>
         * 
         * NOTE: it is not guaranteed that more than one lock instance can be acquired
         * within the same JVM process at any given moment (e.g., if locking is
         * simulated via sockets), although more than one instance can always be
         * created.
         * 
