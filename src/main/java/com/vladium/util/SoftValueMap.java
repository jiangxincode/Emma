 ...
 * implementation does not do internal synchronization for the sake of
 * efficiency]. java.util.ConcurrentModificationException is not supported
 * either.
 *
...
public final class SoftValueMap implements Map {
    ...
    /**
     * Equivalent to
     * <CODE>SoftValueMap(11, 0.75F, getClearCheckFrequency, putClearCheckFrequency)</CODE>.
     */
    ...
    /**
     * Constructs a SoftValueMap with specified initial capacity, load factor, and
     * cleared value removal frequencies.
     *
     * @param initialCapacity          initial number of hash buckets in the table
     *                                 [may not be negative, 0 is equivalent to 1].
     * @param loadFactor               the load factor to use to determine rehashing
     *                                 points [must be in (0.0, 1.0] range].
     * @param readClearCheckFrequency  specifies that every readClearCheckFrequency
     *                                 {@link #get} should check for and remove all
     *                                 mappings whose soft values have been cleared
     *                                 by the garbage collector [may not be less
     *                                 than 1].
     * @param writeClearCheckFrequency specifies that every writeClearCheckFrequency
     *                                 {@link #put} should check for and remove all
     *                                 mappings whose soft values have been cleared
     *                                 by the garbage collector [may not be less
     *                                 than 1].
     */
    ...
    /**
     * Returns the number of key-value mappings in this map. Some of the values may
     * have been cleared already but not removed from the table.
     * <P>
     *
     * <B>NOTE:</B> in contrast with the java.util.WeakHashMap implementation, this
    ...
    /**
     * Returns 'false' is this map contains key-value mappings (even if some of the
     * values may have been cleared already but not removed from the table).
     * <P>
     *
     * <B>NOTE:</B> in contrast with the java.util.WeakHashMap implementation, this
     * is a constant time operation.
    ...
    /**
     * Returns the value that is mapped to a given 'key'. Returns null if (a) this
     * key has never been mapped or (b) a previously mapped value has been cleared
     * by the garbage collector and removed from the table.
     *
    ...
    /**
     * Updates the table to map 'key' to 'value'. Any existing mapping is
     * overwritten.
     *
     * @param key   mapping key [may not be null].
     ...
     *         existed or its value has been cleared by the garbage collector and
    ...
    public Object put(final Object key, final Object value) {
        ...
        // detect if 'key' is already in the table [in which case, set 'currentKeyEntry'
        // to point to its entry]:
        ...
        if (currentKeyEntry != null) {
            ...
            final Object currentKeyValue = ref.get(); // can be null already [no need to work around the get() bug,
                                                      // though]

            if (currentKeyValue == null)
                ref.m_bucketIndex = -1; // disable removal by removeClearedValues() [need to do this because of the
                                        // identity comparison there]
    ...
    public Object remove(final Object key) {
        ...
        for (SoftEntry entry = buckets[bucketIndex], prev = null; entry != null; prev = entry, entry = entry.m_next) {
            ...
            if ((IDENTITY_OPTIMIZATION && (entryKey == key))
                ...
                // [regardless of whether the value has been enqueued or not, disable its
                // processing by removeClearedValues() since the entire entry is removed here]
                ref.m_bucketIndex = -1;
    ...
    public void clear() {
        ...
        for (int b = 0, bLimit = buckets.length; b < bLimit; ++b) {
            for (SoftEntry entry = buckets[b]; entry != null;) {
                ...
                // [regardless of whether the value has been enqueued or not, disable its
                // processing by removeClearedValues()]
    ...
    void debugDump(final StringBuffer out) {
        if (out != null) {
            ...
            out.append("size = " + m_size + ", bucket table size = " + m_buckets.length + ", load factor = "
                    + m_loadFactor + EOL);
            out.append("size threshold = " + m_sizeThreshold + ", get clear frequency = " + m_readClearCheckFrequency
                    + ", put clear frequency = " + m_writeClearCheckFrequency + EOL);
    ...
    /**
     * An extension of WeakReference that can store an index of the bucket it is
     * associated with.
     */
    ...
    static class SoftEntry {
        SoftEntry(final ReferenceQueue valueReferenceQueue, final Object key, Object value, final SoftEntry next,
            ...
            m_softValue = new IndexedSoftReference(value, valueReferenceQueue, bucketIndex); // ... do not retain a
                                                                                             // strong reference to the
                                                                                             // value
            value = null;
    ...
    /**
     * Re-hashes the table into a new array of buckets. During the process cleared
     * value entries are discarded, making for another efficient cleared value
     * removal method.
     */
    private void rehash() {
        // TODO: it is possible to run this method twice, first time using the 2*k+1
        // prime sequencer for newBucketCount
        // and then with that value reduced to actually shrink capacity. As it is right
        // now, the bucket table can
        // only grow in size
        ...
        for (int b = 0, bLimit = buckets.length; b < bLimit; ++b) {
            for (SoftEntry entry = buckets[b]; entry != null;) {
                ...
                // skip entries whose keys have been cleared: this also saves on future
                // removeClearedValues() work
                if (entryValue != null) {
                    // [assertion: 'softValue' couldn't have been enqueued already and can't be
                    // enqueued until strong reference in 'entryKey' is nulled out]
                    ...
                    // adjust bucket index:
                    // [regardless of whether 'softValue' has been enqueued or not, disable its
                    // removal by removeClearedValues() since the buckets get restructured]
                    ref.m_bucketIndex = -1;
                }
        ...
        if (DEBUG) {
            if (m_size > newSize)
                System.out.println("DEBUG: rehash() cleared " + (m_size - newSize) + " values, new size = " + newSize);
        }
    ...
    /**
     * Removes all entries whose soft values have been cleared _and_ enqueued. See
     * comments below for why this is safe wrt to rehash().
     */
    private void removeClearedValues() {
        int count = 0;

        next: for (Reference _ref; (_ref = m_valueReferenceQueue.poll()) != null;) {
            // remove entry containing '_ref' using its bucket index and identity
            // comparison:
            ...
            {
                // [assertion: this reference was not cleared when the last rehash() ran and so
                // its m_bucketIndex is correct]
    ...
    private final ReferenceQueue m_valueReferenceQueue; // reference queue for all references used by SoftEntry objects
                                                        // used by this table
    private final float m_loadFactor; // determines the setting of m_sizeThreshold
    private final int m_readClearCheckFrequency, m_writeClearCheckFrequency; // parameters determining frequency of
                                                                             // running removeClearedKeys() by get() and
                                                                             // put()/remove(), respectively
