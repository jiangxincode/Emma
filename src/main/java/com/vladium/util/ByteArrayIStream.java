...
public final class ByteArrayIStream extends InputStream {
    ...
    public ByteArrayIStream(final byte[] buf, final int length) {
        if ($assert.ENABLED)
            $assert.ASSERT((length >= 0) && (length <= buf.length), "invalid length: " + length);
    ...
    /**
     * Differs from the contruct for InputStream.reset() in that this method always
     * resets the stream to the same it was immediately after creation.
     */
