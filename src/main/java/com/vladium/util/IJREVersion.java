 ...
 * is more sophisticated then a mere check of "java.version" or
 * "java.class.version" and similar system properties because some JVMs allow
 * overriding those by the user (e.g., from the command line). This
 * implementation relies on the core classes only and tries to minimize the
...
public interface IJREVersion {
    ...
    /*
     * Use a dummy nested class to fake a static initializer for the outer interface
     * (I want IJREVersion as an interface and not a class so that all JRE_XXX
     * constants could be imported via "implements").
     */
