...
public abstract class ClassDefParser {
     ...
     */
    public static ClassDef parseClass(final byte[] bytes) throws IOException {
        if (bytes == null)
     ...
     * Parses an array of bytecodes into a {@link ClassDef}.
     */
    public static ClassDef parseClass(final byte[] bytes, final int length) throws IOException {
        if (bytes == null)
            throw new IllegalArgumentException("null input: bytes");
     ...
     */
    public static ClassDef parseClass(final InputStream bytes) throws IOException {
        if (bytes == null)
            throw new IllegalArgumentException("null input: bytes");
     ...
     * work that needs to be done is not complicated, but is rather monotonous --
     * see Chapter 4 of VM spec 1.0 for the class file format.
     */
    private static final class classParser {
        ...
        void constant_pool() throws IOException {
            ...
            if (DEBUG)
                System.out.println("constant_pool_count = " + constant_pool_count + " [actual number of entries = "
            ...
            for (int index = 1; index < constant_pool_count; ++index) {
                ...
                if (DEBUG)
                    System.out.println("[" + index + "] constant: " + cp_info);
        ...
        void this_class() throws IOException {
            ...
            if (DEBUG)
                System.out.println("this_class: [" + _class_index + ']');
        ...
        void super_class() throws IOException {
            final int _class_index = m_bytes.readU2();
            if (DEBUG)
                System.out.println("super_class: [" + _class_index + ']');
        ...
        void interfaces() throws IOException {
            ...
            if (DEBUG)
                System.out.println("interfaces_count = " + _interfaces_count);

            for (int i = 0; i < _interfaces_count; i++) {
                int _interface_index = m_bytes.readU2();
                if (DEBUG)
                    System.out.println("[" + i + "] interface: " + _interface_index);
        ...
        void fields() throws IOException {
            ...
            if (DEBUG)
                System.out.println("fields_count = " + _fields_count);
                    ...
                    if (((field_info.getAccessFlags() & SERIAL_VERSION_UID_FIELD_MASK) == SERIAL_VERSION_UID_FIELD_MASK)
                        ...
                        for (int a = 0, aLimit = attributes.size(); a < aLimit; ++a) {
                            ...
                            if (attr_info instanceof ConstantValueAttribute_info) {
                                final CONSTANT_literal_info constant_value = ((ConstantValueAttribute_info) attr_info)
                                        .getValue(m_table);
        ...
        void methods() throws IOException {
            ...
            if (DEBUG)
                System.out.println("methods_count = " + _methods_count);
        ...
        void attributes() throws IOException {
            ...
            if (DEBUG)
                System.out.println("attributes_count = " + _attributes_count);
