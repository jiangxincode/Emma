...
public interface IConstantCollection extends Cloneable, IClassFormatOutput {
     ...
     * {@link IConstantCollection#iterator()}. It allows iterating over all entries
     * in a way that steps over all 'invalid' inner slots (extra slots consumed by
    ...
    interface IConstantIterator {
         ...
         * 
         * @return int next valid slot index [always positive for a valid slot; -1 when
         ...
         * and replaces the entry that was visited last without invalidating the
         * iterator.
         */
    ...
    /**
     * Returns a CONSTANT_info at a given pool index. Note that 'index' is 1-based
     * [the way an index would be embedded in bytecode instructions]. Note that
     * because CONSTANT_Long and CONSTANT_Double entries occupy two consequitive
     * index slots certain index values inside the valid range can be invalid; use
     * {@link #iterator()} to iterate only over valid entries in a transparent
     * fashion.
     * 
     * @param index constant pool index [must be in [1, size()] range]
     * @return CONSTANT_info constant pool entry at this index [never null]
     * 
     * @throws IllegalStateException     if an attempt is made to reference an
     *                                   invalid slot index
     * @throws IndexOutOfBoundsException if an attempt is made to reference a slot
     ...
     * 
     * @return IConstantIterator iterator over all entries in the collection [never
     *         null]
     */
     ...
     * semantics expressed by 'comparator'. This method guarantees that when
     * comparator.equals(c) is called c.type() is 'type'. The cost is O(pool size).
     * When multiple matches exist, the location of the first one found will be
     * returned (chosen in some indeterministic way).
     * 
    ...
    /**
     * Convenience method that can lookup CONSTANT_Utf8 entries in O(1) time on
     * average. Note that .class format does not guarantee that all such entries are
     * not duplicated in the pool. When multiple matches exist, the location of the
     * first one found will be returned (chosen in some indeterministic way).
     * 
    ...
    /**
     * Appends 'constant' to the end of the collection. No duplicate checks are
     * made.
     * 
    ...
    /**
     * Replaces an existing constant pool entry. A replacement can be made only for
     * a constant of the same width as the constant currently occupying the slot.
     * 
     * @param index    constant pool index [must be in [1, size()] range]
     ...
     * @throws IllegalStateException     if an attempt is made to reference an
     *                                   invalid slot index [see {@link #get(int)}]
     * @throws IndexOutOfBoundsException if an attempt is made to reference a slot
     *                                   outside of the valid range
     */
