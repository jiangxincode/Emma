 ...
 * contains the JVM instructions and auxiliary information for a single Java
 * method, instance initialization method, or class or interface initialization
 * method. Every Java Virtual Machine implementation must recognize Code
 * attributes. There must be exactly one Code attribute in each method_info
 * structure.
 * <P>
 * 
 ...
 * </PRE>
 *
 * The value of the max_stack item gives the maximum number of words on the
 * operand stack at any point during execution of this method.
 * <P>
 * 
 * The value of the max_locals item gives the number of local variables used by
 * this method, including the parameters passed to the method on invocation. The
 * index of the first local variable is 0 . The greatest local variable index
 * for a one-word value is max_locals-1 . The greatest local variable index for
 * a two-word value is max_locals-2.
 * <P>
 * 
 ...
 * array must not be empty.The code array gives the actual bytes of Java Virtual
 * Machine code that implement the method.
 * <P>
 * 
 * The value of the exception_table_length item gives the number of entries in
 * the exception_table table. Each entry in the exception_table array describes
 * one exception handler in the code array: see {@link Exception_info}.
 * <P>
 * 
 * The value of the attributes_count item indicates the number of attributes of
 * the Code attribute. Each value of the attributes table must be a
 * variable-length attribute structure. A Code attribute can have any number of
...
public final class CodeAttribute_info extends Attribute_info {
    ...
    public CodeAttribute_info(final int attribute_name_index, final int max_stack, int max_locals, final byte[] code,
            final IExceptionHandlerTable exceptionHandlerTable, final IAttributeCollection attributes) {
        super(attribute_name_index,
                8 + (code != null ? code.length : 0) + exceptionHandlerTable.length() + attributes.length());
    ...
    public String toString() {
        ...
        s.append("CodeAttribute_info: [attribute_name_index = " + m_name_index + ", attribute_length = "
    ...
    CodeAttribute_info(final IConstantCollection constants, final int attribute_name_index, final long attribute_length,
        ...
        for (int i = 0; i < exception_table_length; ++i) {
            ...
            if (DEBUG)
                System.out.println("\t[" + i + "] exception: " + exception_info);
