 ...
 * are constant pool indices of
 * {@link com.vladium.jcd.cls.constant.CONSTANT_Class_info} structures
 * corresponding to direct superinterfaces of this class/interface. The order in
 * which they appear is the left-to-right order of their declaration in the
 * implements/extends clause.
 * 
...
public interface IInterfaceCollection extends Cloneable, IClassFormatOutput {
     ...
     * pool index for offset'th direct superinterface.
     * 
    ...
    /**
     * Appends a new superinterface pointer to the collection. No duplicate checks
     * are made.
     * 
     * @param interface_index constant pool index [must be positive; input not
     *                        validated]
     ...
     * found in the constant pool, in the slot pointed to by 'interface_index'.
     * 
     * @param offset          offset of the superinterface pointer to replace [must
     *                        be in [0, size()) range]
     * @param interface_index constant pool index [must be positive; input not
     *                        validated]
