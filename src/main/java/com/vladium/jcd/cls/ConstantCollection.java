    ...
    public CONSTANT_info set(final int index, final CONSTANT_info constant) {
        ...
        if (result.width() != constant.width())
            throw new IllegalArgumentException(
                    "assertion failure: can't set entry of type [" + result.getClass().getName()
                            + "] to an entry of type [" + result.getClass().getName() + "] at pool slot " + index);
        ...
        public CONSTANT_info set(final CONSTANT_info constant) {
            ...
            if (result == null) // this should never happen with iterators
                throw new IllegalStateException(
                        "assertion failure: dereferencing an invalid constant pool slot " + m_prev_index);

            if (result.width() != constant.width())
                throw new IllegalArgumentException("assertion failure: can't set entry of type ["
                        + result.getClass().getName() + "] to an entry of type [" + result.getClass().getName()
                        + "] at pool slot " + m_prev_index);
        ...
        if (m_CONSTANT_Utf8_index == null) {
            ...
            for (int i = 0; i < m_constants.size(); ++i) {
                ...
                if ((constant != null) && (constant.tag() == CONSTANT_Utf8_info.TAG)) {
                    // it's ok to always put: the later indices will simply override the earlier
                    // ones
