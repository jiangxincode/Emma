 ...
 * {@link com.vladium.jcd.parser.ClassDefParser} produces from bytecode. Most
 * elements are either settable or extendible. This class also implements
 * {@link com.vladium.jcd.compiler.IClassFormatOutput} and works with
 * {@link com.vladium.jcd.compiler.ClassWriter} to produce bytecode without an
 * external compiler.
 * <P>
 * 
 * MT-safety: this class and all interfaces used by it are not safe for access
        ...
        if (result != 0L)
            ...
            try {
                ...
                // however, ACC_SUPER must be ignored:
                {
                    ...
                    final int modifiers = (isNested(nestedAccessFlags) ? nestedAccessFlags[0] : getAccessFlags())
                ...
                // (3) The name of each interface sorted by name written using UTF encoding
                {
                    ...
                    for (int i = 0; i < iLimit; ++i) {
                        // [in Java format]
                        ifcs[i] = Types.vmNameToJavaName(
                                ((CONSTANT_Class_info) m_constants.get(interfaces.get(i))).getName(this));
                    }
                ...
                // c. The descriptor of the field in UTF encoding
                {
                    ...
                    for (int f = 0, fLimit = fds.length; f < fLimit; ++f) {
                        ...
                        if (((modifiers & ACC_PRIVATE) == 0) || ((modifiers & (ACC_STATIC | ACC_TRANSIENT)) == 0))
                            fds[fcount++] = new FieldDescriptor(field.getName(this), modifiers,
                                    field.getDescriptor(this));
                    }
...
//                             ((hash1 >>> 24) & 0xFF) << 32 | ((hash1 >>> 16) & 0xFF) << 40 | ((hash1 >>> 8) & 0xFF) << 48 | ((hash1 >>> 0) & 0xFF) << 56;
    ...
    public Method_info newEmptyMethod(final String name, final String descriptor, final int access_flags) {
        ...
        final CodeAttribute_info code = new CodeAttribute_info(attribute_name_index, 0, 0,
                CodeAttribute_info.EMPTY_BYTE_ARRAY, AttributeElementFactory.newExceptionHandlerTable(0),
                ElementFactory.newAttributeCollection(0));
    ...
    public int addFieldref(final Field_info field) {
        ...
        final CONSTANT_NameAndType_info nametype = new CONSTANT_NameAndType_info(field.m_name_index,
                field.m_descriptor_index);
