...
public abstract class Types {
    ...
    /**
     * Returns 'c''s package name [does not include trailing '.'] or "" if 'c' is in
     * the default package.
     */
    ...
    /**
     * Converts Java-styled package/class name to how it would be represented in the
     * VM.
     * <P>
     * 
     * Example:<BR>
    ...
    /**
     * Converts a VM-styled package/class name to how it would be represented in
     * Java.
     * <P>
     * 
     * Example:<BR>
    ...
    /**
     * Converts a method signature to its VM descriptor representation. See $4.3 of
     * the VM spec 1.0 for the descriptor grammar.
     * <P>
     * 
     * Example:<BR>
     * signatureToDescriptor(new Object().getClass().getMethod("equals" ,new
     * Class[0])) = "(Ljava/lang/Object;)Z"
     * <P>
     * 
     * Equivalent to
    ...
    /**
     * Converts a method signature (parameter types + return type) to its VM
     * descriptor representation. See $4.3 of the VM spec 1.0 for the descriptor
     * grammar.
     * <P>
     */
    ...
    public static String[] descriptorToParameterTypes(String methoddescriptor) {
        ...
        /*
         * final java.util.Vector _result = new java.util.Vector (); final StringBuffer
         * token = new StringBuffer ();
         * 
         * char c = '*'; int scan = 0;
         * 
         * for (int state = 0; state != 4; ) { try { switch (state) { case 0: c =
         * methoddescriptor.charAt (scan++); if (c == '(') state = 1; else throw new
         * IllegalArgumentException ("malformed method descriptor: [" + methoddescriptor
         * + "]"); break;
         * 
         * case 1: c = methoddescriptor.charAt (scan); switch (c) { case 'B': case 'C':
         * case 'D': case 'F': case 'I': case 'J': case 'S': case 'Z': token.append (c);
         * _result.addElement (token.toString ()); token.setLength (0); scan++; break;
         * 
         * case 'L': state = 2; token.append (c); scan++; break;
         * 
         * case '[': state = 3; token.append (c); scan++; break;
         * 
         * case ')': if (token.length () > 0) { _result.addElement (token.toString ());
         * token.setLength (0); } state = 4; break;
         * 
         * 
         * default: throw new IllegalArgumentException ("[state = " + state + ", c = " +
         * c + "] malformed method descriptor: [" + methoddescriptor + "]");
         * 
         * } // end of nested switch break;
         * 
         * case 2: c = methoddescriptor.charAt (scan++); token.append (c); if (c == ';')
         * { _result.addElement (token.toString ()); token.setLength (0); state = 1; }
         * break;
         * 
         * case 3: c = methoddescriptor.charAt (scan++); token.append (c); if (c != '[')
         * { state = 1; } break;
         * 
         * } // end of switch
         * 
         * //System.out.println ("[state = " + state + ", c = " + c + "]"); } catch
         * (StringIndexOutOfBoundsException e) { throw new IllegalArgumentException
         * ("malformed method descriptor: [" + methoddescriptor + "]"); } }
         * 
         * String [] result = new String [_result.size ()]; _result.copyInto (result);
         * 
         * return result;
         */
    }

    public static String signatureToMethodDescriptor(final String[] parameterTypeDescriptors,
    ...
    public static String fullMethodDescriptorToUserName(final String classJavaName, String methodName,
    ...
    public static String fullMethodDescriptorToFullUserName(final String classJavaName, String methodName,
            final String methoddescriptor) {
        if ("<init>".equals(methodName))
            methodName = simpleClassName(classJavaName);
        if ("<clinit>".equals(methodName))
    ...
    private static final class signatureCompiler {
        ...
        private void parameterType(Class _parameterType) {
            if (_parameterType != null) {
                ...
                {
                    if (byte.class == _parameterType)
                        emit('B');
                    else if (char.class == _parameterType)
                        emit('C');
                    else if (double.class == _parameterType)
                        emit('D');
                    else if (float.class == _parameterType)
                        emit('F');
                    else if (int.class == _parameterType)
                        emit('I');
                    else if (long.class == _parameterType)
                        emit('J');
                    else if (short.class == _parameterType)
                        emit('S');
                    else if (boolean.class == _parameterType)
                        emit('Z');
                } else if (_parameterType.isArray()) // array type:
                {
                    emit('[');
                    parameterType(_parameterType.getComponentType());
                } else // object type:
                {
                    emit('L');
                    emit(javaNameToVMName(_parameterType.getName()));
                    emit(';');
                }
            }
        }
