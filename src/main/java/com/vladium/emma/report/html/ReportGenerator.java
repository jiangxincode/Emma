...
public final class ReportGenerator extends AbstractReportGenerator implements IAppErrorCodes {
    ...
    // TODO: make sure relative file names are converted to relative URLs in all
    // anchors/hrefs
    ...
    public Object visit(final AllItem item, final Object ctx) {
        ...
        try {
            ...
            /*
             * { final HTMLTable.IRow first = statsTable.newRow (); // stats always
             * available
             * 
             * first.newCell ().setText ("total packages: " + item.getChildCount (), true);
             * first.newCell ().setText ("total classes: " + item.getAggregate
             * (IItem.TOTAL_CLASS_COUNT), true); first.newCell ().setText ("total methods: "
             * + item.getAggregate (IItem.TOTAL_METHOD_COUNT), true);
             * 
             * if (m_srcView && m_hasSrcFileInfo) { final HTMLTable.IRow second =
             * statsTable.newRow ();
             * 
             * final HTMLTable.ICell cell1 = second.newCell (); cell1.setText
             * ("total source files: " + item.getAggregate (IItem.TOTAL_SRCFILE_COUNT),
             * true);
             * 
             * if (m_hasLineNumberInfo) { final HTMLTable.ICell cell2 = second.newCell ();
             * 
             * cell2.setText ("total executable source lines: " + item.getAggregate
             * (IItem.TOTAL_LINE_COUNT), true); cell2.getAttributes ().set
             * (Attribute.COLSPAN, "2"); } else { cell1.getAttributes ().set
             * (Attribute.COLSPAN, "3"); } } }
             */
            page.add(statsTable);
    ...
    public Object visit(final PackageItem item, final Object ctx) {
        ...
        try {
            ...
            final HTMLTable childSummaryTable = new HTMLTable("100%", null, null, "0");
            {
                ...
                boolean odd = true;
                final ItemComparator order = m_typeSortComparators[m_srcView ? SrcFileItem.getTypeMetadata().getTypeID()
...
//                    cell.setText ("class " + child.getName () + " methods:", true);
    ...
    private int[] addHeaderRow(final IItem item, final HTMLTable table, final int[] columns) {
        ...
        // determine the set of columns actually present in the header [may be narrower
        // than 'columns']:
...
//    }
            ...
            if (srcFile != null) {
                ...
                try {
                    in = new BufferedReader(
                            new InputStreamReader(new FileInputStream(srcFile), m_settings.getInEncoding()),
                            IO_BUF_SIZE);
                    ...
                    for (String line; (line = in.readLine()) != null; ++l) {
                        ...
                        if (anchorMap != null) {
                            ...
                            if (anchor != null) {
                                ...
                                // a.getAttributes ().set (Attribute.ID, anchor); ID anchoring does not work in
                                // NS 4.0
    ...
    private static final String CSS = " TABLE,TD,TH {border-style:solid; border-color:black;}"
            ...
            + " {background:" + TITLE_BACKGROUND + ";color:white;}" + " TABLE." + CSS_HEADER_FOOTER + " TD." + CSS_NAV
            + " {background:" + NAV_BACKGROUND + ";color:white;}" +

            " ." + CSS_NAV + " A:link {color:white;}" + " ." + CSS_NAV + " A:visited {color:white;}" + " ." + CSS_NAV
            ...
            " TABLE." + CSS_SOURCE + " TD {padding-left:0.25em;padding-right:0.25em;}" + " TABLE." + CSS_SOURCE + " TD."
            + CSS_LINENUM + " {padding-left:0.25em;padding-right:0.25em;text-align:right;background:"
...
//        " TH {border-width: 1px 1px 1px 0px; }" +        
            ...
            + CSS_CLASS_ITEM_SPECIAL + " TD {border-width:1px 1px 1px 0;}" + " TR." + CSS_CLASS_ITEM_SPECIAL + " TD."
            + CSS_DATA_HIGHLIGHT + " {color:red;border-width:1px 1px 1px 0;}" + " TR." + CSS_CLASS_ITEM_SPECIAL + " TD."
            + CSS_DATA_FIRST + " {border-width:1px 1px 1px 1px;}" + " TR." + CSS_CLASS_ITEM_SPECIAL + " TD."
            + CSS_DATA_HIGHLIGHT_FIRST + " {color:red;border-width:1px 1px 1px 1px;}" +

            " TD." + CSS_BLANK + " {border-style:none;background:transparent;line-height:50%;} " + " TD." + CSS_BOTTOM
            + " {border-width:1px 0 0 0;background:transparent;line-height:50%;}" + " TR." + CSS_ODDROW
