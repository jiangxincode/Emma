...
public final class ReportGenerator extends AbstractReportGenerator implements IAppErrorCodes {
    ...
    public Object visit(final PackageItem item, final Object ctx) {
        ...
        try {
            final ItemComparator childrenOrder = m_typeSortComparators[m_srcView
    ...
    private void emitStatsCount(final String name, final int value) throws IOException {
        eol();
        openElementTag(name);
    ...
    private void emitItem(final IItem item, final ItemComparator childrenOrder) throws IOException {
        ...
        final boolean deeper = (childrenOrder != null) && (m_settings.getDepth() > metadata.getTypeID())
     ...
     */
    private void emitItemCoverage(final IItem item, final int[] columns) throws IOException {
    ...
    private void openElementTag(final String tag) throws IOException {
        m_out.write('<');
        m_out.write(tag);
    }

    private void closeElementTag(final boolean simple) throws IOException {
        if (simple)
            m_out.write("/>");
        else
            m_out.write('>');
    ...
    private void endElement(final String tag) throws IOException {
        m_out.write("</");
        m_out.write(tag);
        m_out.write('>');
    }
