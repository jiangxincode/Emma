...
final class ReportDataModel implements IReportDataModel {
        ...
        if (view != null)
            ...
            if (srcView && !m_mdata.hasSrcFileData())
                throw new IllegalStateException(
                        "source file data view requested for metadata with incomplete SourceFile debug info");
            ...
            for (Iterator /* ClassDescriptor */ descriptors = m_mdata.iterator(); descriptors.hasNext();) {
                ...
                if (packageItem == null) {
                    final String packageName = packageVMName.length() == 0 ? "default package"
                            : Descriptors.vmNameToJavaName(packageVMName);
                ...
                final ClassItem classItem = srcView ? new ClassItem(srcfileItem, cls, coverage)
                        : new ClassItem(packageItem, cls, coverage);
                final MethodDescriptor[] methods = cls.getMethods();

                // TODO: handle edge case when all methods of a class have METHOD_NO_BLOCK_DATA
                // set
                for (int m = 0; m < methods.length; ++m) {
                    final MethodDescriptor method = methods[m];

                    if ((method.getStatus() & IMetadataConstants.METHOD_NO_BLOCK_DATA) != 0)
                        continue;

                    // TODO: wouldn't it be more consistent to simply pass the entire descriptor
                    // into MethodItems? (eval mem savings)
                    final MethodItem methodItem = new MethodItem(classItem, m, method.getName(), method.getDescriptor(),
                            method.getFirstLine());
                    // TODO: need to fold class's name into a method name prefix for collapsing case
                    // [only when it is not the same as the file name]
    ...
    ReportDataModel(final IMetaData mdata, final ICoverageData cdata) {
        ...
        // mdata and cdata as soon as all possible views have been constructed and
        // cached
