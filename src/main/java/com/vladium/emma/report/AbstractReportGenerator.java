...
public abstract class AbstractReportGenerator extends AbstractItemVisitor implements IReportGenerator {
    ...
    public void initialize(final IMetaData mdata, final ICoverageData cdata, final SourcePathCache cache,
        ...
        if (debugInfoWarning && m_log.atINFO()) {
            ...
            m_log.info(
                    "showing up to " + MAX_DEBUG_INFO_WARNING_COUNT + " classes without full debug info per package:");
        ...
        for (int t = 0; t < allTypes.length; ++t) {
            ...
            m_typeSortComparators[t] = ItemComparator.Factory.create(orderedAttrIDsWithDir.values(),
    ...
    protected ItemComparator[] m_typeSortComparators; // m_typeSortComparators [t] is a comparator representing the sort
                                                      // order for item type t
    protected int[] m_metrics; // -1 means no pass/fail check for this attribute

    protected Logger m_log; // every report generator is used on a single thread but the logger needs to be
                            // run()-scoped
