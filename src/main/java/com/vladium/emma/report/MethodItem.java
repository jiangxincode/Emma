...
public final class MethodItem extends Item {
    ...
    public MethodItem(final IItem parent, final int ID, final String name, final String descriptor,
            final int firstLine) {
        super(parent);
    ...
    public int getAggregate(final int type) {
        ...
        if (value < 0) {
            ...
            if ((status & IMetadataConstants.METHOD_NO_BLOCK_DATA) != 0) {
                if ($assert.ENABLED)
                    $assert.ASSERT(false, "excluded method in report data model");
    ...
    static {
        METADATA = new Item.ItemMetadata(IItemMetadata.TYPE_ID_METHOD, "method",
                1 << IItemAttribute.ATTRIBUTE_NAME_ID | 1 << IItemAttribute.ATTRIBUTE_METHOD_COVERAGE_ID
