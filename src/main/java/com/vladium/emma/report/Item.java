...
abstract class Item implements IItem {
    ...
    public final IItemAttribute getAttribute(final int attributeID, final int unitsID) {
        // if ($assert.ENABLED) $assert.ASSERT ((attributeID & getMetadata
        // ().getAttributeIDs ()) != 0, "invalid attribute ID [" + attributeID + "] for
        // type [" + getMetadata ().getTypeID () + "]");
    ...
    protected static final class ItemMetadata implements IItemMetadata {
        ...
        ItemMetadata(final int typeID, final String typeName, final long attributeIDs) {
            if ($assert.ENABLED)
                $assert.ASSERT(typeID >= TYPE_ID_ALL && typeID <= TYPE_ID_METHOD, "invalid type ID: " + typeID);
            if ($assert.ENABLED)
                $assert.ASSERT(typeName != null, "typeName = null");
