...
public abstract class ReportProperties implements IAppErrorCodes {
    ...
    public static final class ParsedProperties {
        public void setInEncoding(final String inEncoding) {
            if ($assert.ENABLED)
                $assert.ASSERT(inEncoding != null, "null input: inEncoding");
        ...
        public void setOutEncoding(final String outEncoding) {
            if ($assert.ENABLED)
                $assert.ASSERT(outEncoding != null, "null input: outEncoding");
        ...
        public void setOutDir(final File outDir) {
            if ($assert.ENABLED)
                $assert.ASSERT(outDir != null, "null input: outDir");
        ...
        public void setOutFile(final File outFile) {
            if ($assert.ENABLED)
                $assert.ASSERT(outFile != null, "null input: outFile");
        ...
        public void setUnitsType(final int unitsType) {
            if ($assert.ENABLED)
                $assert.ASSERT(unitsType >= IItemAttribute.UNITS_COUNT && unitsType <= IItemAttribute.UNITS_INSTR,
                        "invalid units type: " + unitsType);
        ...
        public void setViewType(final int viewType) {
            if ($assert.ENABLED)
                $assert.ASSERT(viewType >= IReportDataView.HIER_CLS_VIEW && viewType <= IReportDataView.HIER_SRC_VIEW,
        ...
        public void setDepth(final int depth) {
            if ($assert.ENABLED)
                $assert.ASSERT(depth >= IItemMetadata.TYPE_ID_ALL && depth <= IItemMetadata.TYPE_ID_METHOD,
                        "invalid depth: " + depth);
        ...
        public void setColumnOrder(final int[] columnOrder) {
            if ($assert.ENABLED)
                $assert.ASSERT(columnOrder != null && columnOrder.length != 0, "null/empty input: outEncoding");
        ...
        public void setSortOrder(final int[] sortOrder) {
            if ($assert.ENABLED)
                $assert.ASSERT(sortOrder != null, "null input: sortOrder");
        ...
        public void setMetrics(final IntIntMap metrics) {
            if ($assert.ENABLED)
                $assert.ASSERT(metrics != null, "null input: metrics");
...
//    }
    ...
    public static ParsedProperties parseProperties(final IProperties properties, final String type) {
        if ($assert.ENABLED)
            $assert.ASSERT(properties != null, "properties = null");
        ...
        {
            final String unitsType = getReportProperty(properties, type, IReportProperties.UNITS_TYPE, true, true,
                    IReportProperties.DEFAULT_UNITS_TYPE);
            result.setUnitsType(IReportProperties.COUNT_UNITS.equals(unitsType) ? IItemAttribute.UNITS_COUNT
                    : IItemAttribute.UNITS_INSTR);

            // TODO: invalid setting not checked
        }
        {
            /*
             * view type is no longer a user-overridable property [it is driven by
             * SourceFile attribute presence]
             * 
             * final String viewType = getReportProperty (properties, type,
             * IReportProperties.VIEW_TYPE, IReportProperties.DEFAULT_VIEW_TYPE);
             * result.setViewType (IReportProperties.SRC_VIEW.equals (viewType) ?
             * IReportDataView.HIER_SRC_VIEW : IReportDataView.HIER_CLS_VIEW);
             */
        ...
        {
            final String hideClasses = getReportProperty(properties, type, IReportProperties.HIDE_CLASSES, true, true,
                    IReportProperties.DEFAULT_HIDE_CLASSES);
        ...
        {
            final String depth = getReportProperty(properties, type, IReportProperties.DEPTH, false, true,
                    IReportProperties.DEFAULT_DEPTH);
            ...
            if (IReportProperties.DEPTH_ALL.equals(depth))
                ...
                throw new EMMARuntimeException(INVALID_PARAMETER_VALUE,
                        new Object[] { IReportProperties.DEPTH, depth });
        }

        if (result.getHideClasses() && (result.getViewType() == IReportDataView.HIER_SRC_VIEW)
        ...
        {
            final String columnList = getReportProperty(properties, type, IReportProperties.COLUMNS, false, true,
        ...
        {
            final String sortList = getReportProperty(properties, type, IReportProperties.SORT, false, true,
                    IReportProperties.DEFAULT_SORT);
        ...
        {
            final String metricList = getReportProperty(properties, type, IReportProperties.METRICS, true, true,
                    IReportProperties.DEFAULT_METRICS);
            ...
            for (StringTokenizer tokenizer = new StringTokenizer(metricList, ","); tokenizer.hasMoreTokens();) {
                ...
                if (separator > 0) // silently ignore invalid entries
                {
                    ...
                    try {
                        ...
                        if ((criterion < 0.0) || (criterion > 101.0))
                            continue;
...
//    } // end of nested class
    ...
    private static String getReportProperty(final IProperties properties, final String type, final String key,
            final boolean allowBlank, final boolean trim) {
        return getReportProperty(properties, type, key, allowBlank, trim, null);
    }

    private static String getReportProperty(final IProperties properties, final String type, final String key,
            final boolean allowBlank, final boolean trim, final String dflt) {
        if ($assert.ENABLED)
            $assert.ASSERT(properties != null, "null input: properties");
        if ($assert.ENABLED)
