...
// ----------------------------------------------------------------------------
/**
 * This Processor implementation acts as a remote control bean ("control
 * console") for the instrumentation {@link com.vladium.emma.rt.RT runtime}. It
 * hides all details of our simple socket-based RPC mechanism and data
 * marshalling.
 * <P>
 * 
 * The interface exposed by CtlProcessor is not interactive. However, since each
 * Processor instance is re-run()able, an interactive console app could be
 * easily built on top of one.
 * 
 * @author Vlad Roubtsov, (C) 2005
 */
public final class CtlProcessor extends Processor implements IAppErrorCodes {
    // public: ................................................................
    ...
    protected void _run(final IProperties toolProperties) {
        ...
        final int port;
        {
            ...
            if (_host == null)
                _host = toolProperties.getProperty(RT.PROPERTY_RT_CONTROL_HOST, RT.DEFAULT_RT_CONTROL_HOST);
        ...
        try {
            ...
            for (int c = 0; c < m_commandSequence.length; ++c) {
                ...
                try {
                    ...
                    throw new EMMARuntimeException(prefix + "RPC failure while executing [" + command.getName() + "]",
                            ioe);
                }
                ...
                case ControlRequest.ID_GET_COVERAGE: {
                    ...
                    if ((cdata == null) || cdata.isEmpty()) {
                        log.info(prefix
                                + "no output created (no coverage data has been collected by the server VM yet)");
                    } else {
                        ...
                        try {
                            ...
                            if (log.atINFO()) {
                                ...
                                log.info(prefix + "local copy of coverage data "
                                        + (cdataOutMerge ? "merged into" : "written to") + " ["
                                        + cdataOutFile.getAbsolutePath() + "] {in " + (send - sstart) + " ms}");
                            }
                        } catch (IOException ioe) {
                            throw new EMMARuntimeException(OUT_IO_FAILURE,
                                    new Object[] { cdataOutFile.getAbsolutePath() }, ioe);
                        }
                    }
                }
                    break;
