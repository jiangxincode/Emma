 ...
 */
public final class RunProcessor extends Processor implements IAppErrorCodes {
    // public: ................................................................

    /**
     * Factory method for this processor.
     * 
     * @param delegate loader that will be used for class delegation by the
     *                 instrumenting class loader created by this RunProcessor [if
     *                 null, delegation is disabled]
     * 
    ...
    public synchronized void run() {
        ...
        RTSettings.setStandaloneMode(false); // an optimization to disable RT's static init code [this line must precede
                                             // any reference to RT]
        // reset runtime [RT loads app properties, creates 'cdata', and launches the
        // runtime controller if needed]:
        RT.reset(new RTSettings.SetActions(RTSettings.FIELD_NEW, RTSettings.FIELD_NEW, RTSettings.FIELD_NULL,
        ...
        final IProperties toolProperties;
        {
            ...
            if (appProperties == null)
                appProperties = EMMAProperties.getAppProperties(); // don't use combine()
    ...
    public synchronized void setReportTypes(final String[] types) {
        ...
        if ($assert.ENABLED)
            $assert.ASSERT(reportTypes != null && reportTypes.length > 0);
    ...
    protected void _run(final IProperties toolProperties) {
        ...
        final IInclExclFilter forcedDelegationFilter = IInclExclFilter.Factory.create(
                toolProperties.getProperty(InstrClassLoader.PROPERTY_FORCED_DELEGATION_FILTER), COMMA_DELIMITERS,
                FORCED_DELEGATION_FILTER_SPECS);
        final IInclExclFilter throughDelegationFilter = IInclExclFilter.Factory.create(
                toolProperties.getProperty(InstrClassLoader.PROPERTY_THROUGH_DELEGATION_FILTER), COMMA_DELIMITERS,
                null);
        ...
        try {
            ...
            if (m_sourcePath != null)
                srcpathCache = new SourcePathCache(m_sourcePath, true); // ignore non-existent source dirs

            // create session data containers:
            ICoverageData cdata = RT.getCoverageData();
            if ($assert.ENABLED)
                $assert.ASSERT(cdata != null, "cdata is null");

            IMetaData mdata = DataFactory.newMetaData(CoverageOptionsFactory.create(toolProperties));

            runnerExitHook = new AppRunnerExitHook(log, m_dumpSessionData, sdataOutFile, sdataOutMerge.booleanValue(),
                    mdata, cdata, m_reportGenerators, srcpathCache, toolProperties);
            ...
            if (m_scanCoveragePath) {
                if (USE_SOFT_CACHE)
                    classIOCache = new SoftValueMap(INIT_CACHE_CAPACITY, 0.75F, SOFT_CACHE_READ_CHK_FREQUENCY,
                            SOFT_CACHE_WRITE_CHK_FREQUENCY);
                else
                    classIOCache = new HashMap(INIT_CACHE_CAPACITY, 0.75F);

                final ClassPathProcessorST processor = new ClassPathProcessorST(m_coveragePath, m_canonical, mdata,
                        m_coverageFilter, classIOCache);
            ...
            // app runner does not need these handles anymore [only the exit hook runner
            // maintains them]:
            srcpathCache = null;
            cdata = null;
            ...
            final ClassLoader appLoader;
            {
                ...
                try {
                    appLoader = new InstrClassLoader(m_delegate, m_coveragePath, forcedDelegationFilter,
                            throughDelegationFilter, loadHook, classIOCache);
            ...
            try {
                ...
                } catch (ExceptionInInitializerError eiie) // this should not happen for INIT_AT_LOAD_TIME=false
                {
                    ...
                    throw new EMMARuntimeException(MAIN_CLASS_LOAD_FAILURE,
                            new String[] { m_appClassName, cause.toString() }, cause);
                } catch (Throwable t) {
                ...
                // ensure that the app is bootstrapped using appLoader:
                {
                    ...
                    if (actualLoader != appLoader) {
                        final String loaderName = actualLoader != null ? actualLoader.getClass().getName()
                                : "<PRIMORDIAL>";

                        throw new EMMARuntimeException(MAIN_CLASS_BAD_DELEGATION,
                                new String[] { IAppConstants.APP_NAME, m_appClassName, loaderName });
                    }
                }
                ...
                try {
                    ...
                    appMain = appClass.getMethod("main", MAIN_TYPE); // Sun JVMs do not seem to require the method to be
                                                                     // declared
                } catch (Throwable t) {
                ...
                if (log.atTRACE1()) {
                    if (appLoader instanceof InstrClassLoader)
                        ((InstrClassLoader) appLoader).debugDump(log.getWriter());
                }
                ...
                if (mainFailure != null) {
                    if (mainFailure instanceof InvocationTargetException) {
                        ...
                        throw new EMMARuntimeException(MAIN_METHOD_FAILURE,
                                new String[] { m_appClassName, cause.toString() }, cause);
                        ...
                        throw new EMMARuntimeException(MAIN_METHOD_NOT_FOUND, new String[] { m_appClassName },
                                mainFailure);
                    } else {
                        throw new EMMARuntimeException(MAIN_METHOD_FAILURE,
                                new String[] { m_appClassName, mainFailure.toString() }, mainFailure);
                    }
                }
            } catch (SecurityException se) {
                throw new EMMARuntimeException(SECURITY_RESTRICTION, new String[] { IAppConstants.APP_NAME }, se);
            ...
            RT.reset(new RTSettings.SetActions(RTSettings.FIELD_NULL, RTSettings.FIELD_NULL, RTSettings.FIELD_NULL,
                    RTSettings.FIELD_NEW_IF_NULL));
        }

        if ($assert.ENABLED)
            $assert.ASSERT(runnerExitHook != null, "reportExitHook = null");
    ...
    private static final class AppRunnerExitHook implements Runnable {
                        ...
                        try {
                            ...
                            if (info) {
                                ...
                                m_log.info("raw session data " + (m_sdataOutMerge ? "merged into" : "written to") + " ["
                                        + m_sdataOutFile.getAbsolutePath() + "] {in " + (end - start) + " ms}");
                            }
                        } catch (Throwable t) {
                            m_dataDumpFailure = t;
                        }
        ...
        AppRunnerExitHook(final Logger log, final boolean dumpRawData, final File sdataOutFile,
                final boolean sdataOutMerge, final IMetaData mdata, final ICoverageData cdata,
                final IReportGenerator[] generators, final SourcePathCache cache, final IProperties properties) {
            if (log == null)
    ...
    private static RuntimeException wrapFailure(final Throwable t) {
        if (Exceptions.unexpectedFailure(t, EXPECTED_FAILURES))
            return new EMMARuntimeException(UNEXPECTED_FAILURE,
                    new Object[] { t.toString(), IAppConstants.APP_BUG_REPORT_LINK }, t);
    ...
    static {
        EXPECTED_FAILURES = new Class[] { EMMARuntimeException.class, IllegalArgumentException.class,
