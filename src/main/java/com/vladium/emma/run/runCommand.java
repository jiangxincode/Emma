            ...
            // process 'args':
            {
                ...
                // process free args:
                {
                    ...
                    if (m_jarMode) {
                        ...
                        if (jarMainClass == null) {
                            exit(true, "failed to load Main-Class manifest attribute from [" + jarfile.getAbsolutePath()
                                    + "]", null, RC_UNEXPECTED);
                            return;
                        }
                ...
                // handle cmd line-level defaults and option interaction
                {
                    if (DEFAULT_TO_SYSTEM_CLASSPATH) {
                        if (m_classpath == null) {
                            // TODO" this is not guaranteed to work (in WebStart etc), so double check if I
                            // should remove this
                            ...
                            if (systemClasspath.length() == 0) {
                                ...
                                usageexit(
                                        "could not infer coverage classpath from 'java.class.path'; use an explicit -cp option",
                                        parser, IOptsParser.SHORT_USAGE);
                                return;
                            }
