...
public final class InstrClassLoadHook implements IClassLoadHook {
    ...
    public InstrClassLoadHook(final IInclExclFilter filter, final IMetaData mdata) {
        ...
        // important to use the same options as the metadata may have been populated
        // earlier:
    ...
    public boolean processClassDef(final String className, final byte[] bytes, final int length, ByteArrayOStream out)
            throws IOException {
        if ($assert.ENABLED) {
        ...
        if ((filter == null) || filter.included(className)) {
            ...
            if (m_instrResult.m_descriptor != null) // null means either the metadata existed already or the class is an
                                                    // interface
            {
