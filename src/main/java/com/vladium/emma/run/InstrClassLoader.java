...
public final class InstrClassLoader extends URLClassLoader {
    ...
    // TODO: [see above as well] need to keep track of res URLs to support [path]
    // exclusion patterns
    // TODO: improve error handling so it is clear when errors come from buggy
    // instrumentation
    ...
    public InstrClassLoader(final ClassLoader parent, final File[] classpath,
            final IInclExclFilter forcedDelegationFilter, final IInclExclFilter throughDelegationFilter,
            final IClassLoadHook hook, final Map cache) throws MalformedURLException {
    ...
    public synchronized final Class loadClass(final String name, final boolean resolve) throws ClassNotFoundException {
        ...
        if (c == null) {
            ...
            if (m_parent != null) {
                try {
                    parentsVersion = m_parent.loadClass(name); // note: it is important that this does not init the
                                                               // class
            ...
            if (c == null) {
                try {
                    ...
                    if (parentsVersion != null) {
                        final boolean delegate = (m_throughDelegationFilter == null)
                                || m_throughDelegationFilter.included(name);

                        if (delegate) {
                            c = parentsVersion;
                            if (trace1)
                                m_log.trace1("loadClass",
                                        "[delegation filter] using parent's version for [" + name + "]");
                        } else
                            throw cnfe;
                    } else
                        throw cnfe;
        ...
        if (resolve)
            resolveClass(c); // this never happens in J2SE JVMs
        return c;
    }
    ...
    protected final Class findClass(final String name) throws ClassNotFoundException {
        ...
        if (entry != null) // cache hit
        {
            ...
            try {
                ...
                if ((m_hook != null) && m_hook.processClassDef(name, bytes, length, baos)) // note: this can overwrite
                                                                                           // 'bytes'
                {
        ...
        } else // cache miss
        {
            ...
            classURL = getResource(classResource); // important to hook into URLClassLoader's overload of this so that
                                                   // Class-Path manifest attributes are processed etc
            ...
            if (classURL == null)
                ...
                try {
                    ...
                    if ((m_hook != null) && m_hook.processClassDef(name, bytes, length, baos)) // note: this can
                                                                                               // overwrite 'bytes'
                    {
                        // the instrumented class definition:
                        bytes = baos.getByteArray();
    ...
    private Class defineClass(final String className, final byte[] bytes, final int length, final URL srcURL) {
        ...
        if (lastDot >= 0) {
            ...
            if (pkg == null) {
                definePackage(packageName, IAppConstants.APP_NAME, IAppConstants.APP_VERSION,
                        IAppConstants.APP_COPYRIGHT, IAppConstants.APP_NAME, IAppConstants.APP_VERSION,
                        IAppConstants.APP_COPYRIGHT, srcURL);
            }
        }
    ...
    private final Logger m_log; // a loader instance is used concurrently but cached its log config at
                                // construction time
