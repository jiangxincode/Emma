...
public final class MergeProcessor extends Processor implements IAppErrorCodes {
    ...
    /**
     * NOTE: there is no setter for merge attribute because this processor always
     * overwrites the out file [to ensure compaction]
     * 
        ...
        try {
                ...
                for (int f = 0; f < m_dataPath.length; ++f) {
                    ...
                    if (_mdata != null) {
                        ...
                        if (mdata == null)
                            ...
                            mdata = (IMetaData) mdata.merge(_mdata); // note: later datapath entries override earlier
                                                                     // ones
                    }
                    ...
                    if (_cdata != null) {
                        ...
                        if (cdata == null)
                            ...
                            cdata = (ICoverageData) cdata.merge(_cdata); // note: later datapath entries override
                                                                         // earlier ones
                    }
            ...
            // write merged data into output file:
            {
                ...
                if (rename) // create a temp out file
                {
                    ...
                    log.warning("all merged data will be written to a temp file first ["
                            + tempDataOutFile.getAbsolutePath() + "]");
                }
                ...
                // persist merged session data:
                {
                    ...
                    try {
                        ...
                        if ((mdata == null) || mdata.isEmpty())
                            DataFactory.persist(cdata, persistFile, false); // never merge to enforce compaction
                                                                            // behavior
                        else if ((cdata == null) || cdata.isEmpty())
                            DataFactory.persist(mdata, persistFile, false); // never merge to enforce compaction
                                                                            // behavior
                        else
                            DataFactory.persist(new SessionData(mdata, cdata), persistFile, false); // never merge to
                                                                                                    // enforce
                                                                                                    // compaction
                                                                                                    // behavior
                    } catch (IOException ioe) {
                    ...
                    {
                        if (!Files.renameFile(tempDataOutFile, canonicalDataOutFile, true)) // overwrite the original
                                                                                            // archive
                        {
                            // error code
                            throw new EMMARuntimeException("could not rename temporary file ["
                                    + tempDataOutFile.getAbsolutePath() + "] to [" + canonicalDataOutFile
                    ...
                    if (log.atINFO()) {
                        ...
                        log.info("merged/compacted data written to [" + canonicalDataOutFile + "] {in " + (end - start)
                                + " ms}");
                    }
                }
            }
