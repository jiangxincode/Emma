...
// ----------------------------------------------------------------------------
/**
 * A Processor is a bean-like object that encapsulates the core functionality of
 * a "tool" in our tool set. All our command line tools and ANT tasks are mere
 ...
 * Note that Processor attributes vary regarding whether they are required or
 * optional (have defaults);</li>
 * <li>the instance is executed via {@link #run()}.</li>
 * </ol>
 * A Processor instance is re-<code>run()</code>able (note that all state set
 * via set <code>set*()</code> methods is retained through each run).
 * <P>
 * 
...
public abstract class Processor {
     ...
     * <ol>
     * <li>calls {@link #validateState()} to ensure all required attributes have
     * been set;</li>
     * <li>creates Processor-specific {@link IProperties properties} by combining
     * {@link EMMAProperties#getAppProperties() app-global} properties with property
     * overrides for this Processor instance;</li>
     * <li>pushes a new {@link Logger} on the thread-local stack and sets it as
     * {@link #m_log} field;</li>
     ...
     *                       overrides set optionally via
     *                       {@link #setPropertyOverrides(Properties)} or
     *                       {@link #setPropertyOverrides(IProperties)} [never null]
     */
    protected abstract void _run(IProperties toolProperties);

    /**
     * Validates attribute settings before each {@link #_run(IProperties)}.
     * <P>
     * 
     * Overridden implementations should chain to
     * <code>super.validateState()</code>.
     * 
