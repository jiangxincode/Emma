...
// ----------------------------------------------------------------------------
/**
 * Base class for all our command line tools (Commands). Commands use the
 * following simple pluggability pattern:
 * <ul>
 * <li>each concrete Command implementation is expected to be in a class with
 * full Java name
 * <em>prefix</em>.<em>name</em>.<em>name</em><code>Command</code>, where
 * <em>prefix</em> is {@link com.vladium.emma.IAppConstants#APP_PACKAGE} and
 * <em>name</em> is the canonical Command name;</li>
 * 
 * <li>each concrete Command implementation should be a public class with a
 * public constructor with <code>(String, String[])</code> signature;</li>
 * 
 ...
 * compile time coupling to this base class and its Factory
 * {@link #create(String, String, String[]) method}.
 * 
...
public abstract class Command {
     ...
     * @param usageName command's usage (as seen on the command line) name [may not
     *                  be null or empty]
     * @param args      command's command line args [null is equivalent to an empty
     *                  array]
     * 
     * @return a new Command instance ready to be {@link #run() run}.
     * 
     * @throws EMMARuntimeException if a Command with canonical name 'name' could
     *                              not be instantiated
     */
    ...
    /**
     * A public version of this constructor is called by the Factory method
     * {@link #create(String, String, String[])}. Subclasses must keep this
     * constructor signature and make it public.
     */
    ...
    /**
     * Implemented by actual Command classes to specify the string that should
     * follow command name prefix when dumping usage. See command classes for
     * examples.
     */
    protected abstract String usageArgsMsg();

    /**
     * Returns this command's canonical name (e.g, runCommand can be mapped to "emma
     * run" or "emmarun" command line tools but its canonical name is simply "run").
    ...
    protected final String getCanonicalCommandName() {
        ...
        return clsName.substring(lastDot + 1, clsName.length() - 7); // strip full class name prefix and "Command"
                                                                     // suffix
    }

    /**
     * Returns a prefix used for usage-related error messaging. This prefix is
     * derived from the end user command line string for this command (e.g, "emma
     * run" or "emmarun").
     */
     ...
     * javadoc comment.
     * 
     * @param loader classloader to use for loading the usage resource [may not be
     *               null]
     */
    protected final IOptsParser getOptParser(final ClassLoader loader) {
        final String canonicalToolName = getCanonicalCommandName();
        final String usageResourceName = IAppConstants.APP_PACKAGE.replace('.', '/') + "/" + canonicalToolName + "/"
                + canonicalToolName + "_usage.res";

        return IOptsParser.Factory.create(usageResourceName, loader, usageMsgPrefix(), USAGE_OPT_NAMES);
    }

    /*
     * The intended "vertical" inheritance order for all user-specified options: (1)
     * dedicated command line options, (2) -D option overrides, (3) -properties file
     * overrides.
     */
    /**
     * A helper method for handling a number of options common to all Commands.
     * Concrete Command classes should call this method first on every option and
     * handle the option themselves only if this method returns 'false'.
     * 
    ...
    protected final boolean processOpt(final IOptsParser.IOpt opt) {
        ...
        if ("exit".equals(on)) // 'exit' should always be first in this else-if chain
        {
            ...
            setPropertyOverride(AppLoggers.PROPERTY_VERBOSITY_FILTER,
                    Strings.toListForm(Strings.merge(opt.getValues(), COMMA_DELIMITERS, true), ','));
            return true;
        }
    ...
    /**
     * This method processes all -D property overrides specified on the command
     * line. To ensure that all -D overrides take precedence over all -properties
     * overrides, this method must be called before
     * {@link #processFilePropertyOverrides()}.
     * 
    ...
    /**
     * This method processes the -properties override if it was specified. It should
     * be called after all individual options have been handled via a loop that
     * calls {@link #processOpt(IOptsParser.IOpt)}, because that will ensure that
     * m_propertyFile has been set (otherwise, this method is a no-op). To ensure
     * that file overrides have lower priority than named property overrides, this
     * method should be called after
     * {@link #processCmdPropertyOverrides(IOptsParser.IOpts)}.
     * <P>
     * 
     * This method calls {@link #exit(boolean, String, Throwable, int)} on any file
     * I/O error.
     * 
    ...
    /**
     * A helper method to emit usage-related messages before a call to
     * {@link #exit(boolean, String, Throwable, int)}.
     * 
     * @param msg    message to precede detailed usage text [if null, no such
     *               message is emitted]
     * @param parser options parser instance used to emit detailed usage text [if
     *               null, no text is emitted]
     * @param level  usage text detail level [@see IOptsParser#usage(PrintWriter,
     *               int, int)]
     */
    ...
    /**
     * A helper method to terminate a Command execution with correct -exit
     * semantics. If rc != RC_OK, this message will terminate either by a call to
     * System.exit(rc) or by throwing an exception, depending on {@link #m_exit}
     * setting.
     * 
     * @param showBuildID flag to indicate that pre-exit message should be prefixed
     *                    with {@link IAppConstants#APP_USAGE_BUILD_ID}
     * @param msg         message to precede exit exception stack traces, if any
     *                    [ignored if null]
     * @param t           exit exception [ignored if null, will be wrapped in an
     *                    EMMARuntimeException if not an instance of that already]
    ...
    protected final void exit(final boolean showBuildID, final String msg, final Throwable t, final int rc)
        ...
        if (msg != null) {
            m_out.print(m_usageCommandName + ": ");
            m_out.println(msg);
        }
        ...
        if (rc != RC_OK) {
            ...
            if (m_exit) {
                ...
                if (t instanceof EMMARuntimeException)
                    ...
                    throw msg != null ? new EMMARuntimeException(msg, t)
                            : new EMMARuntimeException("unexpected failure: ", t);
            }
        } else {
     ...
     * @return true if the option has no values [e.g., '-f'] or if its first value
     *         evaluates to 'true' according to {@link Property#toBoolean(String)}.
     */
     ...
     */
    protected static String[] getListOptValue(final IOptsParser.IOpt opt, final String delimiters,
            final boolean processAtFiles) throws IOException {
    ...
    protected final PrintWriter m_out; // stdout writer used by Command code (this is independent from Logger tracing
                                       // done by Processors by design)

    protected File m_propertyFile; // set when '-p'|props|properties' option is processed by processOpt()
    protected Properties m_propertyOverrides; // set by processCmdPropertyOverrides() [combines the effects of
                                              // '-p'|props|properties' and '-D' property overrides]
    protected boolean m_exit; // 'true' if '-exit' has been requested on the command line [set by
                              // processOpt()]
    ...
    /*
     * Instantiate a Command according to the pluggability pattern and using
     * 'loader' as the class loader.
     */
    private static Command instantiate(final String name, final String usageName, final String[] args,
            final ClassLoader loader) throws EMMARuntimeException {
        ...
        try {
            ...
            throw new EMMARuntimeException(IAppErrorCodes.INVALID_COMMAND_NAME,
                    new String[] { IAppConstants.APP_NAME, name }, e);
        } catch (ExceptionInInitializerError eiie) {
            throw new EMMARuntimeException(IAppErrorCodes.INVALID_COMMAND_NAME,
                    new String[] { IAppConstants.APP_NAME, name }, eiie);
        }
    }
