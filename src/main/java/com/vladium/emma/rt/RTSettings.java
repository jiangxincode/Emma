...
// ----------------------------------------------------------------------------
/**
 * Conceptually, this class is an extention of class RT. This is a separate
 * class, however, to help load RT in a lazy manner (before RT's clinit
 * executes).
 * 
...
public abstract class RTSettings {
    ...
    /**
     * This cryptic class holds a set of actions for (re)setting several fields in
     * class RT. See {@link RT#reset(RTSettings.SetActions)}.
     * 
     * @see #mapSetAction(int)
     */
    public static final class SetActions {
        public SetActions(final int appPropertiesAction, final int coverageDataAction, final int exitHookAction,
        ...
        public static boolean[] mapSetAction(final int action) {
            if ($assert.ENABLED)
                $assert.ASSERT(action >= FIELD_NEW_IF_NULL && FIELD_NULL <= 3);
    ...
    /**
     * Returns 'true' if RT should assume standalone mode. This is essentially the
     * same as offline instrumentation mode: instrumented bytecode is running
     * outside of {@link com.vladium.emma.run.RunProcessor run processor} control.
     */
