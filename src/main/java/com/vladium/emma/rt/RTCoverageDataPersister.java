...
// ----------------------------------------------------------------------------
/**
 * A static helper API for coverage data I/O used by
 * {@link com.vladium.emma.rt.RT}, {@link com.vladium.emma.rt.RTExitHook}, and
 * {@link com.vladium.emma.rt.RTController}.
 * 
...
abstract class RTCoverageDataPersister {
     ...
     */
    static void dumpCoverageData(final ICoverageData cdata, final boolean useSnapshot, final File outFile,
            final boolean merge, final IFileLock lock) {
        try {
            if (cdata != null) {
                ...
                final long start = info ? System.currentTimeMillis() : 0;
                {
                    ...
                    synchronized (Object.class) // fake a JVM-global critical section when multilply loaded RT's write
                                                // to the same file
                    {
                        try {
                            ...
                            if (lock != null) {
                                if (trace1)
                                    log.trace1("dumpCoverageData",
                                            "locking coverage output file [" + filePath + "] using " + lock + " ...");
                                else if (info)
                                    log.info("locking coverage output file [" + filePath + "] ...");
                                ...
                                try {
                                    ...
                                    log.log(ILogLevels.SEVERE, "lock for coverage data file [" + filePath
                                            + "] could not be acquired: " + lock, ae);
                                }
                            }
                            ...
                            if (lock != null) {
                                lock.release();
                                if (trace1 && dump)
                                    log.trace1("dumpCoverageData", "output file unlocked");
                            }
                        }
                    }
                }
                ...
                if (dump) {
                    if (info) {
                        ...
                        log.info("runtime coverage data " + (merge ? "merged into" : "written to") + " [" + filePath
                                + "] {in " + (end - start) + " ms}");
                    }
                } else {
