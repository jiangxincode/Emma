...
// ----------------------------------------------------------------------------
/**
 * RTControllerClientProxy is a client-side interface to
 * {@link com.vladium.emma.rt.RTController}. It abstracts away the details of
 * remotely communicating with our instrumentation runtime,
 * {@link com.vladium.emma.rt.RT}.
 * <P>
 * 
 * Lifecycle model:
 * <ol>
 * <li>an instance is acquired using the {@link #create(String, int) Factory}
 * method;</li>
 * <li>the same instance can then {@link #execute(Request)} several RPC
 * requests. A TCP socket connection is only active when a request is being
 * processed.</li>
 * </ol>
 * 
...
public final class RTControllerClientProxy {
     ...
     * endpoint.
     * 
     * @param host server host name in
     *             {@link InetAddress#getByName(java.lang.String)} format [may not
     *             be null]
     * @param port server port [must be in must be in [1, 65535] range]
     * 
     * @return a new proxy instance
     */
    public static RTControllerClientProxy create(final String host, final int port) throws UnknownHostException {
        if (host == null)
    ...
    public Response execute(final Request request) throws IOException {
        ...
        try {
            DataOutputStream out = new DataOutputStream(
                    new BufferedOutputStream(socket.getOutputStream(), OUTPUT_IO_BUF_SIZE));

            Request.write(request, out);
            out.flush();
            out = null;
            if (trace1)
                log.trace1(method, "request [" + request + "] sent");
            ...
            try {
                ...
                if (trace1)
                    log.trace1(method, "reading response ...");
                final Response response = Response.read(in);
                if (trace1)
                    log.trace1(method, "response received [" + response + "]");
    ...
    private RTControllerClientProxy(final String host, final int port) throws UnknownHostException {
        m_port = port;
