...
// ----------------------------------------------------------------------------
/**
 * RT class is the hub through which instrumented bytecode communicates with our
 * runtime. A typical sequence of events for an instrumented class is:
 * <ol>
 * <li>on first class use (which is usually the static initializer but can be
 * any other method in some rare cases) it creates its basic block coverage map
 * array;</li>
 ...
 * the presence of multiple classloading. In some cases, if RT is multiply
 * loaded it will function fine. However, if a runtime
 * {@link com.vladium.emma.rt.RTController controller} is employed (default),
 * multiple RT instances will conflict on the same controller TCP port.
 * 
...
public abstract class RT implements IAppConstants {
    ...
    public static synchronized void reset(final RTSettings.SetActions actions) {
        ...
        // if requested, reload the app properties [to accomodate thread context
        // classloader rearrangements]:
            ...
            if (steps[1] && (s_exitHook == null)) // set the field to new value?
            {
                ...
                if (sm != null) {
                    ...
                    try {
                        ...
                        throw new Error(IAppConstants.APP_NAME
                                + ": current security permissions are too restrictive for file I/O, aborting");
                    }
                }

                // configure the new exit hook with the currently active coverage data and dump
                // file settings:
                final File outFile = getCoverageOutFile();
                final Runnable exitHook = new RTExitHook(RT.class, s_cdata, outFile, getCoverageOutMerge(),
                        getCoverageOutFileLock(outFile));
        ...
        if (steps[1] && (s_controller == null)) // set the field to new value?
        {
            ...
            if (controller != null) {
                ...
                try {
                    ...
                    cause = se;
                    failure = new Error(IAppConstants.APP_NAME
                            + ": current security permissions are too restrictive for socket I/O, aborting");
                } catch (Exception e) {
                    cause = e;
                ...
                if (failure != null) {
                    ...
                    // disable the exit hook since we are about to ask the VM to terminate with no
                    // real coverage collected:
     ...
     * 
     * @param coverage    basic block coverage map created by an instrumented class
     *                    [not null]
     * @param classVMName class name (JVM format), used as the key of this coverage
     *                    entry
     * @param stamp       class bytecode signature [used to detect coverage
     *                    data/metadata inconsistencies]
     */
    public static void r(final boolean[][] coverage, final String classVMName, final long stamp) {
        // note that we use class names, not the actual Class objects, as the keys here.
        // This
        ...
        // ['cdata' can be null if a previous call to dumpCoverageData() disabled data
        // collection]
    ...
    /**
     * This synchronized getter is used by the run processor to get a handle to the
     * same coverage data as being used by the runtime.
     * 
     * @return current coverage data handle [can be null depending on previous calls
     *         to reset()]
     */
    ...
    /**
     * This synchronized getter is used by the run processor to get a handle to the
     * same application properties as being used by the runtime.
     * 
     * @return current application properties [can be null depending on previous
     *         calls to reset() and/or success of loading the properties]
     */
     ...
     * 
     * @param outFile            output file handle [if null, the default as
     *                           determined by app properties will be used]
     * @param merge              output merge flag
     * @param stopDataCollection [if true, no more coverage data will be collected
     *                           and this method will no-op on subsequent
     *                           invocations]
     * 
    ...
    public static synchronized void dumpCoverageData(File outFile, final boolean merge,
        ...
        if (stopDataCollection)
            s_cdata = null; // TODO: log this NOTE: this does not really stop data collection, merely
                            // prevents new class registration
    ...
    /**
     * Equivalent to
     * <code>dumpCoverageData(outFile, app_default_for_merge_flag, stopDataCollection)</code>
     * 
     * @deprecated use {@link com.vladium.emma.ctl.CtlProcessor} instead
     */
    ...
    /*
     * A safe (never fails) method for figuring out the coverage data filename based
     * on the known app properties and their defaults.
     */
    ...
    /**
     * A safe (never fails) method for figuring out the coverage output file lock
     * setup based on the known app properties and their defaults.
     * 
            ...
            if (appProperties != null) {
                ...
                if (property != null) {
                    try {
                        ...
                        System.err.println(
                                "ignoring malformed [" + PROPERTY_RT_FILELOCK_MAX_TIME + "] value: " + property);
                    }
                }
                ...
                if (property != null) {
                    try {
                        ...
                        System.err.println(
                                "ignoring malformed [" + PROPERTY_RT_FILELOCK_RETRIES + "] value: " + property);
                    }
                }
                ...
                if (property != null) {
                    try {
                        ...
                        System.err.println(
                                "ignoring malformed [" + PROPERTY_RT_FILELOCK_PORTBASE + "] value: " + property);
                    }
                }
            }
    ...
    /**
     * A safe (never fails) method for creating the runtime controller based on the
     * known app properties and their defaults.
     * 
    ...
    static {
        if (DEBUG)
            System.out.println(
                    "RT[" + System.identityHashCode(RT.class) + "]::<clinit>: loaded by " + RT.class.getClassLoader());
        ...
        if (RTSettings.isStandaloneMode()) {
            if (DEBUG)
                System.out.println("RT::<clinit>: STANDALONE MODE");

            // load app props, create coverage data, register an exit hook, launch the rt
            // controller:
            reset(new RTSettings.SetActions(RTSettings.FIELD_NEW, RTSettings.FIELD_NEW, RTSettings.FIELD_NEW,
                    RTSettings.FIELD_NEW));
        } else {
            // load app props only (the run processor takes care of coverage data and
            // runtime controller):
            reset(new RTSettings.SetActions(RTSettings.FIELD_NEW, RTSettings.FIELD_NULL, RTSettings.FIELD_NULL,
