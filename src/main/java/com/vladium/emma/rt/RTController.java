...
// ----------------------------------------------------------------------------
/**
 * A remotely accessible controller for {@link com.vladium.emma.rt.RT}.
 * <P>
 * 
 * Lifecycle model:
 * <ol>
 * <li>an instance is {@link #RTController(int) created};</li>
 * <li>the instance is {@link #start() started}. This creates the actual request
 * TCP listen port and the necessary request handling threads.</li>
 * <li>the same instance can then {@link #execute(Request) execute} any number
 * of RPC requests;</li>
 * <li>the controller is (asyncronously) {@link #shutdown() shut down} and can
 * be garbage collected.</li>
 * </ol>
 * 
 * Threading model: {@link #execute(Request)} is called serially from an
 * instance of ExecuteThread (this thread serializes RPC requests that arrive
 * asynchronously on the listen port). This executor thread is different from
 * the threads that call {@link #start()} or {@link #shutdown()}.
 * {@link #shutdown()} can be called asynchronously with respect to request
 * execution and in general will abort the current request (possibly resulting
 * in errors seen on the client side).
 * <P>
 * 
...
final class RTController {
    ...
    /**
     * Creates a server socket listening on {@link RT#PROPERTY_RT_CONTROL_PORT
     * control} port and allocates/starts the request listening and execution
     * threads.
     */
    synchronized void start() throws IOException {
        ...
        final Thread listenThread = new Thread(controllerThreadGroup, listener,
                IAppConstants.APP_NAME + " runtime listen thread");
        listenThread.setDaemon(true);
        m_listener = listener;

        final ExecuteThread executor = new ExecuteThread(this, m_queue);
        final Thread executeThread = new Thread(controllerThreadGroup, executor,
                IAppConstants.APP_NAME + " runtime execute thread");
    ...
    /**
     * Closes the controller listen socket, closes all request queue response
     * sockets, and terminates the request listening and execution threads.
     * <P>
     * 
    ...
    /**
     * This method is not synchronized by design, to make the lifecycle API
     * asynchronous with respect to request execution.
     * <P>
     * 
     * NOTE: since the request queue is handled by a single executor thread, all
     * requests are executed serially (but usually on a different thread from the
     * one that executes start()/shutdown()).
     */
        ...
        try {
            ...
            case ControlRequest.ID_GET_COVERAGE: {
                ...
                if (cdata != null) {
                    ...
                    if (disableShutdownHook) {
                        // TODO: this should be done after data has been safely marshalled back to the
                        // client
                        RT.reset(new RTSettings.SetActions(RTSettings.FIELD_NEW_IF_NULL, RTSettings.FIELD_NEW_IF_NULL,
                                RTSettings.FIELD_NULL, RTSettings.FIELD_NEW_IF_NULL));
                    }
                }
            ...
            case ControlRequest.ID_DUMP_COVERAGE: {
                ...
                if (cdata != null) {
                    ...
                    final File outFile = args[0] != null ? new File(args[0]) : RT.getCoverageOutFile();

                    final boolean outMerge = args[1] != null ? Property.toBoolean(args[1]) : RT.getCoverageOutMerge();

                    final boolean disableShutdownHook = args[2] != null ? Property.toBoolean(args[2]) : true;
                    ...
                    trace = "runtime coverage data remotely " + (outMerge ? "merged into" : "written to") + " ["
                            + outFile.getAbsolutePath() + "] {in " + (end - start) + " ms}";
    ...
    private static final class RequestDescriptor {
        RequestDescriptor(final long timestamp, final Socket socket, final Request request) {
        ...
        public void run() {
            ...
            while (!(shutdownSignalled() || Thread.interrupted())) {
                ...
                try {
                    ...
                    if (trace1)
                        log.trace1(method, "[" + timestamp + "] enqueued new request " + request.getID() + " @"
            ...
            while (!(shutdownSignalled() || Thread.interrupted())) {
                ...
                try {
                    try {
                        rd = (RequestDescriptor) m_queue.dequeue();
                        if (trace1)
                            log.trace1(method, "dequeued request " + rd.m_request.getID());
                    ...
                    // to ensure I/O interruptibility in old JVMs, expose the current request's
                    // socket:
                    synchronized (this) {
                    ...
                    try {
                        out = new DataOutputStream(
                                new BufferedOutputStream(m_socket.getOutputStream(), OUTPUT_IO_BUF_SIZE));
    ...
    private Class m_RT; // keep our RT class pinned in memory (this is actually unnecessary if the
                        // controller is created by RT itself)
