...
public final class InstrVisitor extends AbstractClassDefVisitor
     ...
     * <li>if 'metadata' is true, the class definition is analysed to create a
     * {@link ClassDescriptor} for the original class definition
     * </ul>
     * This method uses a caller-provided out-parameter for returning results. Note
     * that if 'out.m_metadata' is not null 'out.m_instrumented' will almost always
     * be the same as 'instrument', with the exception of classes that can't be
     * instrumented (e.g, due to being at the JVM class size limits).
     * <P>
     * 
     ...
     * class definition, it should have created one as a <code>cls.clone()</code> or
     * by retaining the original definition bytes. This part of contract is for
     * efficienty and also simplifies the implementation.
     * <P>
     * 
     * TODO: "don't instrument" execution path isn't implemented yet (m_instrument
     * is not used)
     * <P>
     * TODO: classes that can't be instrumented aren't detected yet
     * <P>
     */
    public void process(final ClassDef cls, final boolean ignoreAlreadyInstrumented, final boolean instrument,
            final boolean metadata, final InstrResult out) {
        ...
        if (cls.isInterface())
            ...
            // TODO: handle classes that cannot be instrumented due to bytecode/JVM
            // limitations
            ...
            if (m_metadata) {
                ...
                out.m_descriptor = new ClassDescriptor(m_classPackageName, m_className, m_classSignature,
                        m_classSrcFileName, m_classMethodDescriptors);
            }
    ...
    public final Object visit(final ClassDef ignore, final Object ctx) {
        ...
        // SF FR 977738: skip empty classes (javac generates these for some code
        // patterns)
        if (m_excludeEmptyClasses && (methods.size() == 0)) {
            ...
            if (trace1)
                m_log.trace1("visit", "skipping empty class");
            return null;
        }
        ...
        // add a Fieldref for the runtime coverage collector field:
        {
            ...
            // note that post-4019 builds can modify this field outside of <clinit>
            // (although
            // it can only happen as part of initializing a set of classes); however, it is
            // legal
        ...
        // add a Methodref for pre-<clinit> method:
        {
            ...
            m_preclinitMethodrefIndex = constants
                    .add(new CONSTANT_Methodref_info(cls.getThisClassIndex(), nametype_index));
        }
        ...
        if (m_doSUIDCompensation) {
            ...
            if (compensate) {
                // compensation not necessary if the original clsdef already controlled it via
                // 'serialVersionUID':
                {
                    ...
                    if (existingSUIDFieldCount > 0) {
                        ...
                        for (int f = 0; f < existingSUIDFieldCount; ++f) {
                            ...
                            if ((field.getAccessFlags()
                                    & (IAccessFlags.ACC_STATIC | IAccessFlags.ACC_FINAL)) == (IAccessFlags.ACC_STATIC
                ...
                if (compensate && (cls.getThisClassIndex() == 0)) // no superclasses [this tool can't traverse
                                                                  // inheritance chains]
                {
            ...
            if (compensate) {
                ...
                final int fieldModifiers = IAccessFlags.ACC_PRIVATE | IAccessFlags.ACC_STATIC | IAccessFlags.ACC_FINAL;
                final IAttributeCollection fieldAttributes = ElementFactory
                        .newAttributeCollection(MARK_ADDED_ELEMENTS_SYNTHETIC ? 2 : 1);
    ...
    public final Object visit(final IMethodCollection methods, final Object ctx) {
        ...
        for (int m = 0; m < originalMethodCount; ++m) {
            ...
            if (trace2)
                m_log.trace2("visit",
                        (method.isSynthetic() ? "synthetic " : "") + "method #" + m + ": [" + m_methodName + "]");

            final boolean isClinit = IClassDefConstants.CLINIT_NAME.equals(m_methodName);

            // TODO: research whether synthetic methods add nontrivially to line coverage or
            // not
            ...
            if (excluded) {
                if (constructMetadata) {
                    m_classMethodDescriptors[m] = new MethodDescriptor(m_methodName, method.getDescriptor(cls),
                            IMetadataConstants.METHOD_EXCLUDED, m_methodBlockSizes, null, 0);
                }
            } else {
                if ((method.getAccessFlags() & (IAccessFlags.ACC_ABSTRACT | IAccessFlags.ACC_NATIVE)) != 0) {
                    if (constructMetadata) {
                        m_classMethodDescriptors[m] = new MethodDescriptor(m_methodName, method.getDescriptor(cls),
                                IMetadataConstants.METHOD_ABSTRACT_OR_NATIVE, m_methodBlockSizes, null, 0);
                    }
                ...
                } else // this is a regular, non-<clinit> method that has bytecode:
                {
                    ...
                    if (isClinit) {
                        ...
                        if (constructMetadata) {
                            if ($assert.ENABLED)
                                $assert.ASSERT(m_classBlockCounts[m_methodID] > 0, "invalid block count for method "
                                        + m_methodID + ": " + m_classBlockCounts[m_methodID]);
                            if ($assert.ENABLED)
                                $assert.ASSERT(
                                        m_methodBlockSizes != null
                                                && m_methodBlockSizes.length == m_classBlockCounts[m_methodID],
                                        "invalid block sizes map for method " + m_methodID);

                            final int[][] methodBlockMetadata = m_classBlockMetadata[m_methodID];
                            final int status = (methodBlockMetadata == null
                                    ? IMetadataConstants.METHOD_NO_LINE_NUMBER_TABLE
                                    : 0);

                            m_classMethodDescriptors[m] = new MethodDescriptor(m_methodName, method.getDescriptor(cls),
                                    status, m_methodBlockSizes, methodBlockMetadata, m_methodFirstLine);
                        }
                    }
                }
            }
        ...
        final boolean instrumentClinit = false; // TODO: make use of this [to limit instrumentation to clinitHeader
                                                // only], take into account whether we added and whether it is synthetic
        ...
        if (m_clinitID >= 0) {
            ...
            // there is no <clinit> defined by the original class: add one [and mark it
            // synthetic]
            ...
            final CodeAttribute_info code = new CodeAttribute_info(attribute_name_index, 0, 0,
                    new byte[] { (byte) _return }, AttributeElementFactory.newExceptionHandlerTable(0),
            ...
            clinit = new Method_info(IAccessFlags.ACC_STATIC | IAccessFlags.ACC_PRIVATE, name_index, descriptor_index,
                    attributes);
        ...
        // visit <clinit>:
        {
            ...
            if (trace2)
                m_log.trace2("visit",
                        (clinit.isSynthetic() ? "synthetic " : "") + "method #" + m_methodID + ": [<clinit>]");
...

        {
            ...
            final ByteArrayOStream buf = new ByteArrayOStream(PRECLINIT_INIT_CAPACITY);
            {
                ...
                final int instrMethodCount = m_classInstrMethodCount; // actual number of methods to instrument may be
                                                                      // less than the size of the block map
            ...
            final CodeAttribute_info code = new CodeAttribute_info(attribute_name_index, 5, 0, // adjust constants if
                                                                                               // the bytecode emitted
                                                                                               // above changes
                    EMPTY_BYTE_ARRAY, AttributeElementFactory.newExceptionHandlerTable(0),
                    ElementFactory.newAttributeCollection(0));
            ...
            final Method_info preclinit = new Method_info(IAccessFlags.ACC_STATIC | IAccessFlags.ACC_PRIVATE,
                    name_index, descriptor_index, attributes);
        ...
        if (constructMetadata) {
            if ($assert.ENABLED)
                $assert.ASSERT(m_classBlockCounts[m_methodID] > 0, "invalid block count for method " + m_methodID + " ("
                        + IClassDefConstants.CLINIT_NAME + "): " + m_classBlockCounts[m_methodID]);
            if ($assert.ENABLED)
                $assert.ASSERT(
                        m_methodBlockSizes != null && m_methodBlockSizes.length == m_classBlockCounts[m_methodID],
                        "invalid block sizes map for method " + m_methodID);
            ...
            if ((m_clinitStatus & IMetadataConstants.METHOD_ADDED) == 0)
                m_classMethodDescriptors[m_methodID] = new MethodDescriptor(IClassDefConstants.CLINIT_NAME,
                        clinit.getDescriptor(cls), m_clinitStatus, m_methodBlockSizes, methodBlockMetadata,
                        m_methodFirstLine);
        }
    ...
    public final Object visit(final CodeAttribute_info attribute, final Object ctx) {
        ...
        for (int ip = 0; ip < codeSize;) {
            ...
            { // "opcode visit" logic:
                ...
                case _lookupswitch: {
                    ...
                    ov = (code[scan] << 24) | ((0xFF & code[++scan]) << 16) | ((0xFF & code[++scan]) << 8)
                            | (0xFF & code[++scan]);
                    leaders.add(ip + ov);

                    // final int npairs = getU4 (code, scan);
                    // scan += 4;
                    final int npairs = ((0xFF & code[++scan]) << 24) | ((0xFF & code[++scan]) << 16)
                            | ((0xFF & code[++scan]) << 8) | (0xFF & code[++scan]);
                    ...
                    for (int p = 0; p < npairs; ++p) {
                        // iv = getI4 (code, scan);
                        // scan += 4;
                        iv = (code[++scan] << 24) | ((0xFF & code[++scan]) << 16) | ((0xFF & code[++scan]) << 8)
                                | (0xFF & code[++scan]);
                        keys[p] = iv;
                ...
                case _tableswitch: {
                    ...
                    final int low = (code[++scan] << 24) | ((0xFF & code[++scan]) << 16) | ((0xFF & code[++scan]) << 8)
                            | (0xFF & code[++scan]);
                    // final int high = getI4 (code, scan + 8);
                    // scan += 12;
                    final int high = (code[++scan] << 24) | ((0xFF & code[++scan]) << 16) | ((0xFF & code[++scan]) << 8)
                            | (0xFF & code[++scan]);
        ...
        for (int bl = 0, br = 0; bl < blockCount; ++bl) {
            ...
            if (_branch_locations.length > br) {
                ...
                if (next_branch_location < next_leader) {
                    ...
                    if ($assert.ENABLED)
                        $assert.ASSERT(instructionMap.get(next_branch_location, intHolder),
                                "no mapping for " + next_branch_location);
                    else
            ...
            if ($assert.ENABLED)
                $assert.ASSERT(block.m_length == 0 || block.m_instrCount > 0,
                        "invalid instr count for block " + bl + ": " + block.m_instrCount);
            if (m_metadata)
        ...
        // actual basic block instrumentation:
        {
            ...
            // update maxstack as needed [it can only grow]:
            {
                ...
                attribute.m_max_stack += methodMaxStack; // this is not precise, but still need to add because the
                                                         // insertion may be happening at the old maxstack point
        ...
        for (int l = 0; l < blockCount; ++l) {
            ...
            if (l + 1 < blockCount) {
                jumpAdjOffsets[l + 1] = _blocks[l].m_first + _blocks[l].m_length; // implies the insertion goes just
                                                                                  // before the branch
            }
    ...
    public final Object visit(final LineNumberTableAttribute_info attribute, final Object ctx) {
        ...
        if (m_metadata) {
            ...
            if (lineCount == 0) {
                ...
                // TODO: this code does not work if there are multiple LineNumberTableAttribute
                ...
                // construct block->line mapping: TODO: is the loop below the fastest it can be
                // done?
        ...
        for (int l = 0; l < lineCount; ++l) {
            ...
            // TODO: make this faster using either table assist or the sorted array in
            // 'sortedLines'
    ...
    private static final class Block {
        ...
        // int m_last; // exclusive offset of the last non-branch instruction [excludes
        // possible control transfer at the end]
        int m_length; // excluding the branch statement [can be 0]
        int m_instrCount; // size in instructions, including the [optional] original branch; [m_insertion
                          // is not counted]

        // NOTE: it is possible that m_first == m_last [the block is empty except for a
        // possible control transfer instr]
...
//        }
        ...
        /**
         * When this is called, all previous blocks have been written out and their
         * m_first have been updated.
         */
    ...
    static abstract class Branch {
        ...
        // TODO: this method must signal when it is necessary to switch to long jump
        // form
    ...
    static final class RET extends Branch // [wide] ret
    {
        ...
        void emit(final EmitCtx ctx) {
            ...
            if (m_varindex <= 0xFF) {
                out.write2(m_opcode, m_varindex); // indexbyte
            } else {
                out.write4(_wide, m_opcode, m_varindex >>> 8, // indexbyte1
    ...
    static final class LOOKUPSWITCH extends Branch {
        ...
        void emit(final EmitCtx ctx) {
            ...
            // padding bytes:
            for (int p = 0, padCount = 3 - (ip & 3); p < padCount; ++p)
                out.write(0);
    ...
    /**
     * TODO: CodeSegment right now must be 100% position-independent code; otherwise
     * it must follow maxlengtt() Branch pattern...
     */
    ...
    static final class clinitHeader extends CodeSegment {
        clinitHeader(final InstrVisitor visitor, final int localVarIndex) {
            ...
            final int instrMethodCount = visitor.m_classInstrMethodCount; // actual number of methods to instrument may
                                                                          // be less than the size of the block map
            if ($assert.ENABLED)
                $assert.ASSERT(blockCounts != null && blockCounts.length >= instrMethodCount,
                        "invalid block count map");
            ...
            buf.write3(_invokestatic, preclinitMethodrefIndex >>> 8, // indexbyte1
                    preclinitMethodrefIndex); // indexbyte2

            // [stack +1]

            // TODO: disable this when there are no real blocks following?
            // [in general, use a different template when this method contains a single
            // block]
        ...
        int maxstack() {
            return 2;
        } // note: needs to be updated each time emitted code changes
        ...
        void emit(final EmitCtx ctx) {
            ...
            try {
                ...
                if ($assert.ENABLED)
                    $assert.ASSERT(false, ioe.toString());
            }
        }
    ...
    static final class methodHeader extends CodeSegment {
        methodHeader(final InstrVisitor visitor, final int localVarIndex) {
            ...
            buf.write4(_getstatic, coverageFieldrefIndex >>> 8, // indexbyte1
            ...
            buf.write3(_ifnonnull, // skip over pre-<clinit> method call
                    0, 3 + /* size of the block below */ 4);

            // [stack +1]

            // block: call pre-<clinit> method
            {
                buf.write4(_pop, _invokestatic, preclinitMethodrefIndex >>> 8, // indexbyte1
                        preclinitMethodrefIndex); // indexbyte2
    ...
    // TODO: use a compilation flag to use table assist here instead of binary
    // search
    // BETTER YET: use binsearch for online mode and table assist for offline [when
    // memory is not an issue]

    /**
     * Returns the maximum index 'i' such that (values[i] <= x). values[] contains
     * distinct non-negative integers in increasing order. values[0] is 0, 'x' is
     * non-negative.
     * 
     * Edge case: returns values.length-1 if values [values.length - 1] < x
     */
    ...
    private int[][][] m_classBlockMetadata; // methodID->(blockID->line) map [valid only if 'm_constructMetadata' is
                                            // true; null if the method has not line number table]
    ...
    /* private */ int m_preclinitMethodrefIndex;
    ...
    /* private */ int m_classInstrMethodCount;
    // the number of slots in 'm_classBlockCounts' corresponding to methods to be
    // instrumented for coverage
    /* private */ int[] m_classBlockCounts; // basic block counts for all methods [only valid just before <clinit> is
                                            // processed]
    ...
    private int[] m_methodBlockOffsets; // [unadjusted] basic block boundaries [length =
    ...
    /*
     * It appears that nested classes and interfaces ought to be marked as
     * Synthetic; however, neither Sun nor IBM compilers seem to do this.
     * 
     * (As a side note, implied no-arg constructors ought to be marked as synthetic
     * as well, but Sun's javac is not consistent about that either)
     */
