...
public abstract class InstrProcessor extends Processor implements IPathEnumerator.IPathHandler {
    ...
    protected void validateState() {
        ...
        if (m_outMode != OutMode.OUT_MODE_OVERWRITE) {
            ...
            // FR_SF988785: detect if the user attempted to use a parent of m_outDir as one
            // of
            ...
            for (File dir = canonicalOutDir; dir != null; dir = dir.getParentFile()) // getParentFile() does no real I/O
            {
                for (int ip = 0; ip < instrPathLength; ++ip) {
                    if (dir.equals(canonicalInstrPath[ip]))
                        throw new IllegalStateException("output directory [" + canonicalOutDir
                                + "] cannot be one of the instrumentation path directories (or a child thereof)");
                }
            }
        }
    ...
    protected final void createDir(final File dir, final boolean mkall) throws EMMARuntimeException {
        if (mkall) {
            if (!dir.mkdirs() && !dir.exists())
                throw new EMMARuntimeException(IAppErrorCodes.OUT_MKDIR_FAILURE,
    ...
    protected final File getFullOutDir(final File pathDir, final boolean isClass) {
        if (m_outMode == OutMode.OUT_MODE_OVERWRITE) {
            ...
            throw new IllegalStateException("invalid out mode state: " + m_outMode);
    }
