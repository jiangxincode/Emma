...
public abstract class EMMAProperties {
    ...
    // [the DEFAULT_xxx settings duplicate the defaults in
    // APP_DEFAULT_PROPERTIES_RES_NAME
    ...
    /**
     * Global method used to create an appearance that all app work has been done at
     * the same point in time (useful for setting archive and report timestamps
     * etc).
     * 
     * @return the result of System.currentTimeMillis (), evaluated on the first
     *         call only
     */
    ...
    public static synchronized IProperties getAppProperties(final ClassLoader loader) {
        ...
        if (properties != null)
            ...
            final IProperties systemRedirects = wrap(
                    Property.getSystemPropertyRedirects(EMMAProperties.SYSTEM_PROPERTY_REDIRECTS));
            ...
            properties = IProperties.Factory.combine(userOverrides,
                    IProperties.Factory.combine(system, IProperties.Factory.combine(systemFile,
                            IProperties.Factory.combine(appDefaults, systemRedirects))));
    ...
    static {
        ...
        redirects.put(IReportProperties.PREFIX.concat(IReportProperties.OUT_ENCODING), "file.encoding");
        redirects.put(IReportProperties.PREFIX.concat(IReportProperties.OUT_DIR), "user.dir");
