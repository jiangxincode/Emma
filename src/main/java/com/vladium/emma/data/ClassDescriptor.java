...
public final class ClassDescriptor implements IConstants, Serializable {
    ...
    public ClassDescriptor(final String packageVMName, final String name, final long stamp, final String srcFileName,
    ...
    // equality is defined based on <m_packageVMName, m_name> only (m_stamp not
    // mixed in by design):
    ...
    public final boolean equals(final Object rhs) {
        ...
        if (!m_name.equals(_rhs.m_name))
            return false;
        if (!m_packageVMName.equals(_rhs.m_packageVMName))
            return false;
    ...
    public String toString(final String indent) {
        StringBuffer s = new StringBuffer(indent + "class ["
                + (m_packageVMName.length() > 0 ? m_packageVMName + "/" : "") + m_name + "] descriptor:");
    ...
    static ClassDescriptor readExternal(final DataInput in) throws IOException {
    ...
    static void writeExternal(final ClassDescriptor cls, final DataOutput out) throws IOException {
