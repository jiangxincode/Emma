...
//    }
            ...
            if (incompleteDebugInfo) {
                ...
                if (log.atINFO()) {
                    ...
                    if (m_packagesWarned.add(packageVMName)) {
                        log.info("package [" + packageVMName + "] contains classes [" + cls.getName()
    ...
    static void writeExternal(final MetaData mdata, final DataOutput out) throws IOException {
    ...
    private MetaData(final CoverageOptions options, final HashMap classMap, final boolean hasSrcFileInfo,
            final boolean hasLineNumberInfo) {
        if ($assert.ENABLED)
            $assert.ASSERT(options != null, "options is null");
        m_options = options;
