...
final class CoverageData implements ICoverageData, Cloneable, Serializable {
            ...
            for (Iterator entries = rhscoverageData.entrySet().iterator(); entries.hasNext();) {
                ...
                if (data == null)
                    ...
                    else // merge two runtime profiles
                    {
                        ...
                        if ($assert.ENABLED)
                            $assert.ASSERT(coverage.length == rhscoverage.length, "coverage.length [" + coverage.length
                                    + "] != rhscoverage.length [" + rhscoverage.length + "]");
                        for (int m = 0, mLimit = coverage.length; m < mLimit; ++m) {
                            ...
                            if (mcoverage == null) {
                                if ($assert.ENABLED)
                                    $assert.ASSERT(rhsmcoverage == null, "mcoverage == null but rhsmcoverage != null");
                                ...
                                if ($assert.ENABLED)
                                    $assert.ASSERT(rhsmcoverage != null, "mcoverage != null but rhsmcoverage == null");
                                if ($assert.ENABLED)
                                    $assert.ASSERT(mcoverage.length == rhsmcoverage.length,
                                            "mcoverage.length [" + mcoverage.length + "] != rhsmcoverage.length ["
                                                    + rhsmcoverage.length + "]");
    ...
    static CoverageData readExternal(final DataInput in) throws IOException {
    ...
    static void writeExternal(final CoverageData cdata, final DataOutput out) throws IOException {
    ...
    private CoverageData(final HashMap coverageMap) {
        if ($assert.ENABLED)
            $assert.ASSERT(coverageMap != null, "coverageMap is null");
        m_coverageMap = coverageMap;
    }

    // extreme Serializable customization: the effect of it is to reduce
    // defaultRead/WriteObject()
    // to reading/writing class metadata only; custom serial data is then denoted
    // by TC_BLOCK_DATA start marker and the above fast versions of
