...
public final class MethodDescriptor implements IConstants, IMetadataConstants, Serializable {
    ...
    public MethodDescriptor(final String name, final String descriptor, final int status, final int[] blockSizes,
            final int[][] blockMap, final int firstLine) {
        if (name == null)
        ...
        if ((status & METHOD_NO_BLOCK_DATA) == 0) {
            ...
            if ((status & METHOD_NO_LINE_DATA) == 0) {
                ...
                if ($assert.ENABLED) {
                    $assert.ASSERT(blockCount == blockMap.length,
                            "blockCount " + blockCount + " != blockMap.length " + blockMap.length);
