...
// ----------------------------------------------------------------------------
/**
 * Persistence and Factory API for metadata and coverage data. This
 * implementation works with custom read/writeExternal() methods (note that
 * those are not Externalizable implementations), which results in significant
 * (3x-5x) serialization speed-up for large amounts of data.
 * <P>
 * 
...
public abstract class DataFactory {
    ...
    public static IMergeable[] load(final File file) throws IOException {
        if (file == null)
    ...
    public static void persist(final IMetaData data, final File file, final boolean merge) throws IOException {
        if (data == null)
    ...
    public static void persist(final ICoverageData data, final File file, final boolean merge) throws IOException {
        if (data == null)
            throw new IllegalArgumentException("null input: data");
        if (file == null)
            throw new IllegalArgumentException("null input: file");
    ...
    public static void persist(final ISessionData data, final File file, final boolean merge) throws IOException {
        if (data == null)
            throw new IllegalArgumentException("null input: data");
        if (file == null)
            throw new IllegalArgumentException("null input: file");
    ...
    public static boolean[] readBooleanArray(final DataInput in) throws IOException {
        final int length = in.readInt();
        if (length == NULL_ARRAY_LENGTH)
            return null;
        else {
    ...
    public static void writeIntArray(final int[] array, final DataOutput out) throws IOException {
        if (array == null)
    ...
    public static void writeBooleanArray(final boolean[] array, final DataOutput out) throws IOException {
        if (array == null)
            out.writeInt(NULL_ARRAY_LENGTH);
        else {
            final int length = array.length;
    ...
    private static final class RandomAccessFileInputStream extends BufferedInputStream {
        ...
        RandomAccessFileInputStream(final RandomAccessFile raf, final int bufSize) throws IOException {
    ...
    private static final class RandomAccessFileOutputStream extends BufferedOutputStream {
        ...
        RandomAccessFileOutputStream(final RandomAccessFile raf, final int bufSize) throws IOException {
     ...
     */
    private static IMergeable[] mergeload(final File file) throws IOException {
        ...
        if (!file.exists()) {
            ...
            try {
                ...
                // 'file' is a valid existing file, but it could still be of 0 length or
                // otherwise corrupt:
                final long length = raf.length();
                if (trace1)
                    log.trace1(method, "[" + file + "]: file length = " + length);

                if (length < FILE_HEADER_LENGTH) {
                    throw new IOException("file [" + file.getAbsolutePath() + "] is corrupt or was not created by "
                    ...
                    if (length > FILE_HEADER_LENGTH) // return {null, null} in case of equality
                    {
                        ...
                        if (magic != MAGIC)
                            throw new IOException("cannot read [" + file.getAbsolutePath() + "]: not created by "
                                    + IAppConstants.APP_NAME);
                        ...
                        if (dataVersion != IAppConstants.DATA_FORMAT_VERSION) {
                            ...
                            if (gotAppVersion) {
                                throw new IOException("cannot read [" + file.getAbsolutePath()
                                        ...
                                        + makeAppVersion(major, minor, build) + "]");
                            } else {
                                throw new IOException("cannot read [" + file.getAbsolutePath()
                                        + "]: created by another " + IAppConstants.APP_NAME + " version");
                            }
                        ...
                        while (true) {
                            ...
                            if ((entryLength <= 0) || (position + entryLength + ENTRY_HEADER_LENGTH > length))
                                ...
                                if (trace2)
                                    log.trace2(method, "[" + file + "]: found valid entry of size " + entryLength
                                            + " and type " + type);
                                {
                                    if (trace2)
                                    ...
                                    if (trace2)
                                        log.trace2(method,
                                                "entry read in " + (System.currentTimeMillis() - entrystart) + " ms");
                                    ...
                                    if (current == null)
                                        ...
                                        result[type] = current.merge(data); // note: later entries overrides earlier
                                                                            // entries
                                }
     ...
     * input checked by the caller
     */
    private static void persist(final IMergeable data, final byte type, final File file) throws IOException {
        final Logger log = Logger.getLogger();
        final boolean trace1 = log.atTRACE1();
        final boolean trace2 = log.atTRACE2();
        ...
        try {
            ...
            if (file.exists()) {
                ...
                final long length = raf.length();
                if (trace1)
                    log.trace1(method, "[" + file + "]: existing file length = " + length);
                ...
                if (length < 4) {
                    ...
                    final int magic = raf.readInt();
                    if (magic != MAGIC)
                        throw new IOException("cannot overwrite [" + file.getAbsolutePath() + "]: not created by "
                                + IAppConstants.APP_NAME);
...
//                        else
                            ...
                            if (dataVersion != IAppConstants.DATA_FORMAT_VERSION) {
                                ...
                                if (length == FILE_HEADER_LENGTH) {
                                    ...
                                    while (true) {
                                        ...
                                        if ((entryLength <= 0)
                                                || (position + entryLength + ENTRY_HEADER_LENGTH > length))
                                            break;
                                        else {
                                            if (trace2)
                                                log.trace2(method,
                                                        "[" + file + "]: found valid entry of size " + entryLength);
                                    ...
                                    if (trace2)
                                        log.trace2(method, "[" + file + "]: adding entry at position " + position);
                ...
                if (trace1)
                    log.trace1(method, "[" + file + "]: creating a new file");
            ...
            if (overwrite) {
                ...
                writeFileHeader(raf);
                if ($assert.ENABLED)
                    $assert.ASSERT(raf.getFilePointer() == FILE_HEADER_LENGTH,
                            "invalid header length: " + raf.getFilePointer());
    ...
    private static void writeFileHeader(final DataOutput out) throws IOException {
        out.writeInt(MAGIC);
    ...
    private static void writeEntryHeader(final DataOutput out, final byte type) throws IOException {
        out.writeLong(UNKNOWN); // length placeholder
        out.writeByte(type);
    }

    private static void writeEntry(final Logger log, final RandomAccessFile raf, final long marker,
            final IMergeable data, final byte type) throws IOException {
        // [unfinished] entry header:
        writeEntryHeader(raf, type);

        // serialize 'data' starting with the current raf position:
        RandomAccessFileOutputStream rafout = new RandomAccessFileOutputStream(raf, IO_BUF_SIZE); // note: no new file
                                                                                                  // descriptors created
                                                                                                  // here
        {
...
//            oout = null;
    ...
    private static IMergeable readEntry(final RandomAccessFile raf, final byte type, final long entryLength)
        ...
        RandomAccessFileInputStream rafin = new RandomAccessFileInputStream(raf, IO_BUF_SIZE); // note: no new file
                                                                                               // descriptors created
                                                                                               // here
        {
...
//            }
        ...
        if ($assert.ENABLED)
            $assert.ASSERT(rafin.getCount() == entryLength,
                    "entry length mismatch: " + rafin.getCount() + " != " + entryLength);

        return (IMergeable) data;
    }

    /*
     * This is cloned from EMMAProperties by design, to eliminate a
     * CONSTANT_Class_info dependency between this and EMMAProperties classes.
     */
