...
// ----------------------------------------------------------------------------
/**
 * A simple Java version-independent logging framework. Each Logger is also an
 * immutable context that encapsulates configuration elements like the logging
 * verbosity level etc. In general, a Logger is looked up as an inheritable
 * thread-local piece of data. This decouples classes and logging configurations
 * in a way that seems difficult with log4j.
 * <P>
 * 
 * Note that a given class is free to cache its context in an instance field if
 * the class is instantiated and used only on a single thread (or a set of
 ...
 * sense to scope a context to a single method invocation.
 * <P>
 * 
 ...
 * <LI>caller-supplied Throwable is dumped starting with a new line, if not
 * null.
 * </OL>
 * 
 * MT-safety: a given Logger instance will not get corrupted by concurrent usage
 * from multiple threads and guarantees that data written to the underlying
...
public final class Logger implements ILogLevels {
    ...
    public static Logger create(final int level, final PrintWriter out, final String prefix, final Set classMask,
        ...
        if (base == null) {
            ...
            final int _level = level >= NONE ? level : base.m_level;

            final PrintWriter _out = (out != null) && !out.checkError() ? out : base.m_out;
...
//                : base.m_prefix;
            ...
            final Set _classMask = classMask != null ? classMask : base.m_classMask;
    ...
    /**
     * A quick method to determine if logging is enabled at a given level. This
     * method acquires no monitors and should be used when calling one of log() or
     * convenience logging methods directly incurs significant parameter
    ...
    /**
     * A convenience method to log 'msg' from an anonymous calling method at WARNING
     * level.
     * 
    ...
    /**
     * A convenience method to log 'msg' from an anonymous calling method at INFO
     * level.
     * 
     * @param msg log message [ignored if null]
     */
    ...
    /**
     * A convenience method to log 'msg' from an anonymous calling method at VERBOSE
     * level.
     * 
     * @param msg log message [ignored if null]
     */
     ...
     * 
     * @param level level to log at [the method does nothing if this is less than
     *              the set level].
     * @param msg   log message [ignored if null]
     */
    ...
    /**
     * Returns the current top of the thread-local logger stack or the static Logger
     * instance scoped to Logger.class if the stack is empty.
     * 
    ...
    /**
     * Requiring a context parameter here helps enforce correct push/pop nesting in
     * the caller code.
     * 
     * @param ctx [may not be null]
     */
    ...
    private void _log(final int level, final String method, final String msg, final boolean logCaller) {
    ...
    private void _log(final int level, final String method, final String msg, final Throwable throwable) {
        if ((level <= m_level) && (level >= SEVERE)) {
    ...
    static {
        ...
        final Properties properties = Property.getAppProperties(IAppConstants.APP_NAME_LC,
        ...
        STATIC_LOGGER = create(level, new PrintWriter(System.out, false), IAppConstants.APP_NAME, filter);
    }
